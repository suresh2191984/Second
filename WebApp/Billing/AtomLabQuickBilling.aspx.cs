...
public partial class Billing_AtomLabQuickBilling : BasePage, IDisposable
{
    public Billing_AtomLabQuickBilling()
        : base("Billing\\AtomLabQuickBilling.aspx")
    {
    }
    ...
    string HideFields = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string Duepatient = string.Empty;
        ...
        Duepatient = GetConfigValue("chkDuepatient", OrgID);
        HideFields = GetConfigValue("Hide Fields for CAYA", OrgID);
        if(Duepatient =="Y")
        {
            hdnckeckdueamt.Value = "Y";
        }
        ...
        if (!IsPostBack)
        {
            ...
            if (strRate == "Y")
            {
                trRate.Attributes.Add("display", "block");
            }
            else
            {
            ...
            if (Request.QueryString["PID"] != null)
            {
                ...
                else
                {
                    ...
                    if (CPEDIT != "Y")
                    {
                        ...
                        //{
                        //    trVisitTypePart.Attributes.Add("style", "display:block");
                        //    trDispType.Attributes.Add("style", "display:block");
                        //    trSampleTRFPart.Attributes.Add("style", "display:block");
                        //}
                    ...
                    else
                    {
                        ...
                        tdAdditionalDetails.Style.Add("display", "table-row");
            ...
            {
                HideFieldsForCAYA();
                tdhideFields.Style.Add("display", "block");
                tdhideFields1.Style.Add("display", "block");
            }
            else
            {
                tdhideFields.Style.Add("display", "none");
                tdhideFields1.Style.Add("display", "none");
            }
            if (!IsPostBack)
            {
                if (BookingNumber != "")
        ...
        trSampleTRFPart.Attributes.Add("class", "hide");
        PnlPatientDetail.Enabled = true;
    }
    ...
    public void loadCollectSampleList(long VisitID, string gUID)
    {
        try
        {
            ...
            if (lstPatientInvestigation.Count() > 0)
            {
                ...
                string CheckboxPanelConfig = string.Empty;
                CheckboxPanelConfig = GetConfigValue("HideCheckboxlist", OrgID);
                if (CheckboxPanelConfig == "N")
                {
    ...
    protected void LoadCountry(List<Country> lstcountries)
    {
        ...
        try
        {
            ...
            //ddCountry.SelectedValue = selectedCountry.CountryID.ToString();
            Int32.TryParse(ddCountry.SelectedItem.Value, out countryID);
           // hdnDefaultCountryID.Value = countryID.ToString();
            LoadState(countryID);
            ...
            if (childItems.Count() > 0)
            {
                ...
                lblMobile.Text = "<u>M</u>obile +" + childItems[0].ISDCode.ToString();
                hdnDefaultCountryStdCode.Value = "+" + childItems[0].ISDCode.ToString();
            }
            else
            {
                lblMobile.Text = "<u>M</u>obile";
            }
        ...
        try
        {
            ...
            if (lstmetadataOutput.Count > 0)
            {
                ...
                if (childItems4.Count() > 0)
                {
                    ...
                    ddlPatientStatus1.DataSource = childItems4;
                    ddlPatientStatus1.DataTextField = "DisplayText";
                    ddlPatientStatus1.DataValueField = "Code";
                    ddlPatientStatus1.DataBind();
                }
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        ...
        {
            PnlPatientDetail.Enabled = false;
        }
        else
        {
            PnlPatientDetail.Enabled = true;
    ...
    public void SaveData()
    {
        try
        {
          ...
          //  PageContextDetails.isActionDisabled = !chkMobileNotify.Checked;
            ...
            if (lstPatientDueChart.Count > 0 || hdnIsEditMode.Value == "Y")
            {
                ...
                new Patient_BL(base.ContextInfo).InsertPatientBilling(objPatient, objFinalBill, Convert.ToInt64(hdnReferedPhyID.Value),
                                                    ...
                                                    out lstBillingDetails, out returnStatus, SavePicture, sVal, RoleID, LID, PageContextDetails,
                                                     //hdnIsReceptionPhlebotomist.Value,   
                                                     //Code commented by arivalagan (insert task while cancel workorder)
                                                    ...
                                                    Convert.ToInt64(hdnClienID.Value), TodayVisitID, IsSamplePickUP, externalVisitID, approvalNo, out taskID,IsCopay, lstPatientDiscount, strMyCardActiveDays, strMemebershipcardType,
                                                    strHealthCardType, strOTP, strStatus, MembershipCardMappingID, strCreditRedeemTye,
                                                    RedeemPatientid, RedeemVisitid, RedeemOrgId, RedemPoints, RedemValue, lstPatientRedemDetails, lstPkgandGrps, StatFlag, ClientFlag, 0, "", "", "", "", visittemplate, objlang,"","","");
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alrUpdateSucess", "alert('Ordered List count is zero so you cannot billed');reloadPage();", true);
            }
            if (lstBillingDetails.Count > 0 && returnStatus >= 0 && hdnIsEditMode.Value == "N")
            {
                ...
                if (hdnIsReceptionPhlebotomist.Value == "Y" && needTaskDisplay == 1)
                {
                    ...
                    trCollectSample.Attributes.Add("display", "block");
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alrUpdateSucess", "javascript:ClearControlValues();", true);
                }
                else if (Request.QueryString["CPEDIT"] == "Y")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alrUpdateSucess", "alert('Updated Sucessfully');javascript:labQuickBilling();", true);
                }
                else
                {
                    string NeedBillInvestigation = GetConfigValue("NeedBillInvestigationSRS", OrgID);

                    string sPage1 = "../Reception/SrsPrint.aspx?pid=" + patientID.ToString()
                 + "&vid=" + patientVisitID.ToString()
                 + "&pagetype=BP&quickbill=N&bid=" + 0 + "&visitPur=" + 0 + "&ClientName=" + 0 + "&OrgID=" + OrgID + "&IsPopup=Y" + "&IsNeedInv=Y";

                    if (NeedBillInvestigation == "Y")
                    {
                        ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_WINDOW", "var Mleft = (screen.width/2)-(760/2);var Mtop = (screen.height/2)-(700/2);window.open( '" + sPage1 + "', null, 'height=700,width=760,status=yes,toolbar=no,scrollbars=yes,menubar=no,location=no,top=\'+Mtop+\', left=\'+Mleft+\'' );", true);
            ...
            else if (hdnIsEditMode.Value == "Y")
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alrUpdateSucess", "alert('Updated Sucessfully');redirectPage();", true);
            }
            else
            {
                if (returnStatus == 101)
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "altSaveFailed", "alert('ExternalVisitID Already Registered With Another Patient');javascript:labQuickBilling();", true);
                }
                else
                {
    ...
    public void PrintBilling(long PatientID, long patientVisitID, long FinalBillID, long vpurpose, string ddlClientName)
    {
        ...
        if (NoNeedBillPrint != "Y")
        {
            ...
            {
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "clear", "javascript:clearPageControlsValue('Y');OpenBillPrint('" + sPage + "');ClearControlValues();", true);
            }
        }
        else
        {
    ...
    public Patient GetPatientDetails()
    {
        ...
        try
        {
            ...
            if (HideFields == "Y")
            {
                patient.PatientStatus = ddlPatientStatus1.SelectedValue;
            }
            else
            {
            ...
            string PasswordForCaya = GetConfigValue("PasswordLength", OrgID);
            int passwordlength = 6;
            if (PasswordForCaya == "Y")
            {
                passwordlength = 4;
            }
            else
    ...
    private long SaveTRFPicture(string number, string visitid)
    {
        ...
        Root = "TRF_Upload-" + OrgID + "-" + Year + "-" + Month + "-" + Day + "-" + "TRF_Upload" + "-";
        Root = Root.Replace("-", "\\\\");
    ...
    {
        ddState.Value = StateID.ToString();
	//	ddState.Value = hdnDefaultStateID.Value;
        txtName.Text = "";
        //txtName.Focus();
