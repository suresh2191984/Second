...
public partial class Deployability_ServiceExclusivity : BasePage
{
    ...
    [WebMethod(EnableSession = true)]
    [System.Web.Script.Services.ScriptMethod()]
    public static string GetOrgInvestigationsGroupandPKG(string prefixText, string contextKey, int orgID)
    {
        Deployability_BL objBL = new Deployability_BL(new BaseClass().ContextInfo);
        List<string> items = new List<string>();
        ...
        objBL.GetOrgInvestigationsGroupandPKGExclusivity(orgID, ClientID, prefixText, ItemsType, out lstBFD);

        var query = from c in lstBFD
                    where c.Descrip.ToLower().Contains(prefixText.ToLower()) || c.ProcedureName.ToLower().Contains(prefixText.ToLower())
                    select new { c.Descrip, c.ID, c.IsHistoryMandatory };
        JavaScriptSerializer js = new JavaScriptSerializer();
        string strout = js.Serialize(query);
        return strout;
    }
    ...
    public static string GetClientMappingServiceExclusivity(string CidAndRefID, int OrgID)
    {
        long returnCode = -1;
        long Client = -1;
        string refType = "";
        ...
        try
        {
            ...
            returnCode = objBL.GetClientMappingServiceExclusivity(OrgID, Client, refType, out lstClientMappingService);
    ...
    public static string btnFinalSaveContents(List<ClientServiceExclusivity> lstCSMDetails, int OrgID, long LID, int ClientID)
    {
        long returnCode = -1;
        try
        {
            ...
            Deployability_BL objBL = new Deployability_BL(new BaseClass().ContextInfo);
                ...
                string RateName = "";
                returnCode = objBL.SaveClientServiceExclusivity(RateName, OrgID, LID, MappingType, lstCSMDetails, ClientID);
