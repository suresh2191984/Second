...
public partial class InPatient_IPViewBill : BasePage
{
    public InPatient_IPViewBill()
        : base("InPatient\\IPViewBill.aspx")
   {
   }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                chkLabPrint.Checked = true;
                chkLabPrint.Text = "Dont Show Lab Items";
                lblLab.Visible = false;
                lblLabValue.Visible = false;
                Rs_TreatmentCharges.Visible = true;
                ...
                new GateWay(base.ContextInfo).GetBillConfigDetails(iBillGroupID, "Preprinter", OrgID, ILocationID, out lstConfig);
                string Preprinter = string.Empty;
                if (lstConfig.Count > 0)
                {
                    Preprinter = lstConfig[0].ConfigValue;
                    string[] s = Preprinter.Split('-');
                    hdnPreprinter.Value = s[0];
                    hdnpreprinterHeight.Value = s[1];
                }
                else
                {
                    hdnPreprinter.Value = "N";
                }
                #endregion
        ...
        try
        {
            ...
            if (lstDueChart.Count > 0)
            {
                ...
                if (IsCreditBill == "Y")
                {
                    ...
                    dueLab = (from lst in lstDueChart
                              where (lst.FeeType == "INV" || lst.FeeType == "PKG" || lst.FeeType == "GRP") && lst.IsReimbursable != "N"
                              select lst.Amount * lst.Unit).Sum();
                }
                else
                {
                    ...
                    dueSum = (from lst in lstDueChart
                              ...
                              select lst.Amount * lst.Unit).Sum();
                    dueLab = (from lst in lstDueChart
                              where (lst.FeeType == "INV" || lst.FeeType == "PKG" || lst.FeeType == "GRP")
                              select lst.Amount * lst.Unit).Sum();
                }
                ...
                lblLabValue.Text = dueLab.ToString("0.00");
            ...
            if (IsCreditBill != "Y" || firstLoad == 0)
            {
                txtGrandTotal.Text = Convert.ToString(Convert.ToDecimal(txtGross.Text) + Convert.ToDecimal(lblServiceCharge.Text) - lstFinalBill[0].DiscountAmount + Convert.ToDecimal(txtTax.Text) - (Convert.ToDecimal(txtPreviousAmountPaid.Text) +
                                   dPayerTotal) //+ Convert.ToDecimal(txtRecievedAdvance.Text)
                                    + Convert.ToDecimal(lblRoundOff.Text));////+ Convert.ToDecimal(txtPreviousDue.Text)
            ...
            {
                lblInvoiceDate.Text = lstFinalBill[0].BillDate.ToString("dd/MMM/yy hh:mm tt");
            }
            ...
            //    else
            //    {
            //        lblTPADue.Text = ((Convert.ToDecimal(txtGross.Text) + Convert.ToDecimal(txtTax.Text) + Convert.ToDecimal(lblRoundOff.Text)) - Convert.ToDecimal(txtThirdParty.Text) - (chkMedicalItems.Checked ? 0 : Convert.ToDecimal(lblPaidAmt.Text))).ToString();
            //    }
            //}
    ...
    protected void gvTreatment_RowDataBound(Object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    List<PatientDueChart> lstchilddues = new List<PatientDueChart>();
                    List<PatientDueChart> childItems = new List<PatientDueChart>();
                    if (IsCreditBill == "Y")
                    {
                         childItems = (from child in lstDueChart
                                                            where child.FeeType == lblFeeType.Text.Trim() && child.IsReimbursable != "N" && lblComments.Text.Trim() == child.Comments.Trim()
                                                            select child).ToList();
                    ...
                    {
                         childItems = (from child in lstDueChart
                                                           where child.FeeType == lblFeeType.Text.Trim()  && lblComments.Text.Trim() == child.Comments.Trim()
                                                           select child).ToList();
                    ...
                    if (sLabEntered <= iCount)
                    {
                        ...
                        if (childItems.Count > 0)
                        {
                            if (lstChildItems.Count > iTempCount)
                            {
                                ...
                                if (IsCreditBill == "Y")
                                {
                                     childSubitems = (from child in childItems
                                ...
                                {
                                     childSubitems = (from child in childItems
                                                                           where (child.Comments.Split('~')[1].ToLower() == lstChildItems[iTempCount].ToLower()) 
                                                                           select child).ToList();
                                }
                ...
                txtGross.Text = (Convert.ToDecimal(txtGross.Text) + Convert.ToDecimal(dtotalAmount)).ToString();
                Label lblFee = (Label)e.Row.FindControl("lblFeeType");
                if (chkLabPrint.Checked == false)
                {
                    if (lblFee.Text == "INV" || lblFee.Text == "GRP" || lblFee.Text == "PKG")
                    {
                        e.Row.Visible = false;
                    }
                }
    ...
    protected void btnViewReceipt_Click(object sender, EventArgs e)
    {
        ...
        {
            string sPath = "InPatient\\\\IPViewBill.aspx.cs_1";
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "alert", "alert('"+ sPath +"');", true);
        }
    ...
    protected void chkLabPrint_CheckedChanged(object sender, EventArgs e)
    {
        if (chkLabPrint.Checked == true)
        {
            chkLabPrint.Text = "Dont Show Lab Items";
            LoadDetails(1);
            lblLab.Visible = false;
        ...
        else
        {
            ...
            chkLabPrint.Text = "Show Lab Items";
            LoadDetails(0);
            lblLab.Visible = true;
            lblLabValue.Visible = true;
            Rs_TreatmentCharges.Visible = false;
        }
    }
