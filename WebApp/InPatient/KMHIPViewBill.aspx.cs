...
public partial class InPatient_KMHIPViewBill : BasePage
{

    public InPatient_KMHIPViewBill()
        : base("InPatient\\KMHIPViewBill.aspx")
   {
   }
    ...
    string IsCorporateBill = string.Empty;
    char s;
    long visitID = 0;
    long patientID = 0;
    int firstLoad = -1;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            { 
                long lngRateid=0;
                Int64.TryParse(Request.QueryString["RateID"].ToString(), out lngRateid);
                IPBulkBillcon.lngRateId = lngRateid;
            }
    ...
    public void LoadDetails(int firstLoad)
    {
        ...
        try
        {
            ...
            {
                if (lstPatientDetail[0].IsSurgeryPatient == "Y")
                {
                    hdnIsSurgeryPatient.Value = "Y";

                }
                else
                {
                    hdnIsSurgeryPatient.Value = "N";
                }
                if (lstPatientDetail[0].NeedServiceBill == "Y")
                {
                    hdnNeedCorpSummaryBill.Value = "1";

                }
                else
                {
                    hdnNeedCorpSummaryBill.Value = "";
                }
                if (lstPatientDetail[0].IsCorporatetBill == "Y")
                {
                    IsCorporateBill = "Y";
                    hdnIsCorporatetBill.Value = "Y";
                    printArea2.Style.Add("Display", "block");
                    chkInvBreakup.Style.Add("Display", "block");
                    chkInvBreakup.Checked = true;
                }
                else if (lstPatientDetail[0].IsCorporatetBill == "N")
                {
                    IsCorporateBill = "N";
                    hdnIsCorporatetBill.Value = "N";
                    printArea2.Style.Add("Display", "block");
                    chkInvBreakup.Style.Add("Display", "block");
                    chkInvBreakup.Checked = true;
                }
                if(lstPatientDetail[0].NeedServiceBill == "N" && lstPatientDetail[0].IsCorporatetBill == "N")
                {
                    IsCorporateBill = "N";
                    hdnIsCorporatetBill.Value = "N";
                    printArea2.Style.Add("Display", "none");
                    chkInvBreakup.Style.Add("Display", "none");
                    chkInvBreakup.Checked = false;
                }
            }



            //string[] ClientList = { "Port Trust", "BSNL", "FCI", "MMTC", "NLC", "ONGC", "NIFT", "STC" };
            //string strConfigval = string.Empty;
            //for (int i = 0; i < ClientList.Length; i++)
            //{
            //    strConfigval = GetConfigValue(ClientList[i], OrgID);
            //    if (!string.IsNullOrEmpty(strConfigval))
            //    {
            //        if (lstPatientDetail[0].ClientID.ToString() == strConfigval)
            //        {
            //            break;
            //        }
            //    }
            ...
            //}
            //string[] ClientList1 = { "Port Trust", "BSNL", "FCI", "MMTC" };
            //string strConfigval1 = string.Empty;
            //for (int i = 0; i < ClientList1.Length; i++)
            //{
            //    strConfigval1 = GetConfigValue(ClientList1[i], OrgID);
            //    if (!string.IsNullOrEmpty(strConfigval1))
            //    {
            //        if (lstPatientDetail[0].ClientID.ToString() == strConfigval1)
            //        {
            //            hdnNeedCorpSummaryBill.Value = "1";
            //            break;
            //        }
            ...
            if (lstDueChart.Count > 0)
            {
                ...
                #region ParentvisitHaveChild Sureshkumar Mohan added
                if (IsVisitHaveChild.ToUpper() == "Y")
                {

                    btnParentandBabyBill.Visible = false;
                }
                ...
                IEnumerable<PatientDueChart> pdcs = (from s in lstBedBooking
                                                     group s by new { s.FeeID, s.RoomTypeName, s.CreatedAt, s.Unit, s.FromDate, s.ToDate,s.ReimbursableAmount,s.NonReimbursableAmount } into g
                                                     select new PatientDueChart
                                                     {
                                                         ...
                                                         ReimbursableAmount=g.Key.ReimbursableAmount,
                                                         NonReimbursableAmount=g.Key.NonReimbursableAmount
            ...
            decimal deRound = getOPCustomRoundoff(Convert.ToDecimal(Convert.ToDecimal(txtGross.Text)));
            //lblRoundOff.Text = String.Format("{0:0.00}",deRound);
            lblRoundOff.Text = String.Format("{0:0.00}", deRound - Convert.ToDecimal(txtGross.Text));
            if (IsCreditBill != "Y" || firstLoad == 0)
            {
            ...
            if (lstPatientDetail[0].DischargedDT.ToString("dd/MM/yyyy").Trim() == "01/01/0001")
                lblInsuranceBillDateText.Text = MaxDate.ToString("dd/MM/yyyy");
            else
                lblInsuranceBillDateText.Text = lstPatientDetail[0].DischargedDT.ToString("dd/MM/yyyy");
            ...
            //if (lstPatientDetail[0].Status == "Y")
            //{
            //    lblInsuranceIPNoText.Text = "-";
            //}
            //else
            //{
            ...
            //    }
            //}
            if (lstPatientDetail[0].Status == "Y" || lstPatientDetail[0].IPNumber.Trim() == "0")
            {
                lblInsuranceIPNoText.Text = "-";
            }
            else
            ...
            //Status variable is assigned in IsDayCare to check it for not view in IPNo
            //if (lstPatientDetail[0].Status == "Y")
            //{
            //    lblIPNo.Text = "-";
            //}
            ...
            if (IsCreditBill == "Y" || firstLoad == 0)
            {
                ...
                btnCopayReceipt.Visible = IsCreditBill == "Y" ? Convert.ToDecimal(lblCoPayment.Text).Equals(decimal.Zero) ? false : true : false;
                #region Try
    ...
    protected void btnViewSummary_Click(object sender, EventArgs e)
    {
        ...
        if (hdnIsCorporatetBill.Value == "Y" && hdnNeedCorpSummaryBill.Value == "1" && hdnIsSurgeryPatient.Value != "Y")
        {
            Response.Redirect("~/InPatient/CreditBill.aspx?PID=" + patientID + "&VID=" + visitID + "&PNAME=&vType=IP&BP=Print");
        }
        else
        {
    ...
    protected void btnCopayReceipt_Click(object sender, EventArgs e)
    {
        long visitID = 0;
        if (Request.QueryString["VID"] != null)
        {
        ...
        returnCode = objBL.GetCoPaymentReceipt(visitID, OrgID, out lstDetails);
    ...
    {
        //IPBulkBillcon.LoadDet("");
        if (chkInvBreakup.Checked == true)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowBreakUp", "fnShowInv('chkInvBreakup');", true);
            chkInvBreakup.Checked = true;
        }
    ...
    protected void chkPharmaPrint_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (chkPharmaPrint.Checked == true)
        {
            ...
            chkPharmaPrint.Text = "Dont Show Pharmacy Items";
            IPBulkBillcon.gvPRMVisible  = true;
           
        }
        else
        {
            gvMedicalItems.Style.Add("display", "none");
            dvpharmacy.Style.Add("align", "right");
            tdpharma.Style.Add("text-align", "right");
            chkPharmaPrint.Text = "Show Pharmacy Items";
            IPBulkBillcon.gvPRMVisible = false;
    ...
    protected void btnParentandBabyBill_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("~/InPatient/IPMergeBill.aspx?PID=" + patientID + "&VID=" + visitID + "&PNAME=&vType=IP&BP=" + BP + "");
