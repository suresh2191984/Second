localStorage.clear();
function loadfilefromApi(filepath)
{
    ...
    open_from_File_Api(filepath, callback)
}
...
function are_you_sure(action, canceled){
	if(saved){
		...
		$w.$Button("Save", function(){
		    if (Annotationpagetype == "Anno_Intraop")
		        Intraopfalg = 1;
		    var ErrorMsg = SListForAppMsg.Get("PlatForm_CommonControls_Error") == null ? "Alert" : SListForAppMsg.Get("PlatForm_CommonControls_Error");
		    var AppMsg;
		    if (imganame.length == 0) {
		        $w.close();
		        AppMsg = SListForAppMsg.Get('PlatForm_CommonControls_ImageAnnotation_ascx_05') != null ? SListForAppMsg.Get('PlatForm_CommonControls_ImageAnnotation_ascx_05') : "Provide name of image";
		        ValidationWindow(AppMsg, ErrorMsg);
		        return false;
		    }
		    $('#ImageAnnotation_txtfilecomments').val(anno_Coments);
		    $w.close();
			file_save();
			action();
		}).focus();
