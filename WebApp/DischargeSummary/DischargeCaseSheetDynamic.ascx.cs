...
public partial class DischargeSummary_DischargeCaseSheetDynamic : BaseControl
{
    #region declaration
    long patientID = -1;
    ...
    public void LoadPatientDischarge(long patientVisitID)
    {

        try
        {
            GetPatientHistory(patientVisitID);
            GetDischargeSummaryCaseSheet(patientVisitID);
        }
        catch (Exception ex)
        {
           
            CLogger.LogError("Error in DischargeCaseSheet.aspx:Page_Load", ex);
        }
    }

    private void GetPatientHistory(long patientVisitID)
    {
        try
        {
