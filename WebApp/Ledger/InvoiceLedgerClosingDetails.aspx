<%@ Page Language="C#" AutoEventWireup="true" CodeFile="InvoiceLedgerClosingDetails.aspx.cs"
    Inherits="Ledger_InvoiceLedgerClosingDetails" EnableEventValidation="false" %>
                                ...
                                <table class="w-100p searchPanel">
                                    <tr class="panelHeader">
                                        <td class="colorforcontent w-100p" height="23" align="center">
                                            <asp:Label ID="Rs_FilterResult2" runat="server" Text="Invoice Ledger Closing" meta:resourcekey="Rs_FilterResult2Resource1"></asp:Label>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
        ...
        try {
            ...
            if (result != "[]") {
                oTable = $('#tblOutstanding').dataTable({
                    ...
                    "aoColumns": [
               { "mData": "Description", sClass: "w-20p a-left", "mRender": function(data, type, full) 
               {
                   if (full.InvoiceId != '0') 
                   {
                       return '<a href="../Ledger/InvoiceLedgerClosingBills.aspx?CID=' + full.InvoiceNumber + '&INVOICEID=' + full.InvoiceId + '&CODE=' + document.getElementById('lblClientName1').innerHTML + '" onclick="javascript:return IsBilldata(' + full.InvoiceId + ');">' + data + '</a>';
                   }
                   else if (full.InvoiceId == '0') 
                   {
                       return '<a href="../Ledger/InvoiceLedgerClosingBills.aspx?CID=' + full.InvoiceNumber + '&INVOICEID=' + full.InvoiceId + '&CODE=' + document.getElementById('lblClientName1').innerHTML + '" onclick="javascript:return IsBilldata(' + full.InvoiceId + ');" style="font-family:Candara; font-size:large; color:Red;">' + data + '</a>';
                   }
               }
               },
               //{ "mData": "Age", sClass: "w-10p a-right" },
                 {"mData": "DR", sClass: "w-10p a-right", "mRender": function(data, type, full) {
                     if (full.InvoiceId != '0') {
                         return '<span id="spnMethodName' + full.ClientId + '">' + data + '</span>'
                     }
                     else if (full.InvoiceId == '0') {
                     return '<span id="spnMethodName' + full.ClientId + '" style="color: Red;font-family:Segoe UI; font-size:large;">' + data + '</span>'
                     }
                 }
             },
             //{ "mData": "Barcode", sClass: "w-10p a-right"},
                 {"mData": "CR", sClass: "w-10p a-right", "mRender": function(data, type, full) {
                     if (full.InvoiceId != '0') {
                         return '<span id="spnMethodName' + full.ClientId + '">' + data + '</span>'
                     }
                     else if (full.InvoiceId == '0') {
                     return '<span id="spnMethodName' + full.ClientId + '" style="color: green;font-family:Segoe UI; font-size:large;">' + data + '</span>'
                     }
                 }
}],
...
<script type="text/javascript">
    ...
    function FunHome() {
        window.location.replace("../Ledger/InvoiceLedgerClosing.aspx");
    }
