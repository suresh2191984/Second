...
public partial class CommonControls_CorpoRatePackageProfileControl : BaseControl
{
    long visitID = -1;
    ...
    public void LoadPackageData()
    {
        ...
        //var Packages = from packageNameCorpoRate in lstPackages
        ...
        if (lstPackages.Count > 0)
        {
            ...
            foreach (InvGroupMaster objGM in lstPackages)
            {
                ...
                if (UseClientID == 0)
                {
                    SvAL = objGM.GroupID.ToString().Trim() + "~" + (objGM.GroupName.Split(new[] { "--" }, StringSplitOptions.None)[0]);// + "~" + (((objGM.GroupName.Split('-')[1]).Replace(CurrencyName, "")).Split(':')[1].Trim());// "-Rs: 3250.00";
                }
                else
                {
                    SvAL = objGM.GroupID.ToString().Trim() + "~" + (objGM.GroupName.Split(new[] { "--" }, StringSplitOptions.None)[0]) + "~" + (((objGM.GroupName.Split(new[] { "--" }, StringSplitOptions.None)[1]).Replace(CurrencyName, "")).Split(':')[1].Trim());// "-Rs: 3250.00";
                }
