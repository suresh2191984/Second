...
public partial class CommonControls_CorpRateConsultationDynamicDetails : BaseControl
{
    ...
    public void LoadSpecialityName()
    {
        ...
        new PatientVisit_BL(base.ContextInfo).CorporateGetSpecialityAndSpecialityName(OrgID, out lstPhySpeciality, VisitRateID, out lstSpeciality);
        //string iscor = Session["IsCorporateOrg"].ToString();
       
  
        if (lstSpeciality.Count > 0)
        {
            hdnSpecialityCount.Value = lstSpeciality.Count.ToString();
            for (int i = 0; i < lstSpeciality.Count; i++)
            {
                hndSpecialityCMD.Value += lstSpeciality[i].SpecialityCommands.ToString() + "^";
            }

        }
        if (lstPhySpeciality.Count > 0)
        {
            hdnPhyCount.Value = lstSpeciality.Count.ToString();
            for (int i = 0; i < lstPhySpeciality.Count; i++)
            {
                hdnPhyCMD.Value += lstPhySpeciality[i].PhysicianSpecialityCommands.ToString() + "^";
            }
