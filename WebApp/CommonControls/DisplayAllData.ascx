<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DisplayAllData.ascx.cs"
    Inherits="UserControl_DisplayAllData" %>
...
<script type="text/javascript" language="javascript">
    ...
    function CmdAddBillItemsType_onclick(FeeType, FeeID, OtherID, Descrip, Quantity, Amount, Total) {
        ...
        if (iPaymentAlreadyPresent == 0) {
            ...
            var userMsg = SListForApplicationMessages.Get('CommonControls\\DisplayAllData.ascx_1');
            if (userMsg != null) {
                alert(userMsg);
            } else {
...
//        }
        ...
        startPaymentTag = "<TABLE ID='tabDrg1' Cellpadding='1' Cellspacing='1' Border='1' style='BackgroundColor:#ff6600;' ><TBODY><tr><td style='display:none;'>"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_FeeType %>"+" </td><td style='display:none;'>"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_FeeID %>"+" </td> <td style='display:none;'>"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_OtherID %>"+"</td> <td >"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_Description %>"+" </td> <td > "+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_Quantity %>"+"</td><td >"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_Amount %>"+" </td><td>"+"<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_Total %>"+"</td><td></td></tr>";
        endPaymentTag = "</TBODY></TABLE>";
...
//                }
                newPaymentTables += "<TD >" + Amount + "</TD>";
                newPaymentTables += "<TD >" + Total + "</TD>";
                newPaymentTables += "<TD><input name='RID^" + 0 + "~FeeType^" + FeeType + "~FeeID^" + FeeID + "~OtherID^" + OtherID + "~Descrip^" + Descrip + "~Quantity^" + Quantity + "~Amount^" + Amount + "~Total^" + Total + "' onclick='btnPaymentDelete_OnClick(name);' value = '<%=Resources.ClientSideDisplayTexts.CommonControls_DisplayAllData_Delete %>' type='button' style='background-color:Transparent;color:Red;border-style:none;text-decoration:underline;cursor:pointer'  /></TD>" +"<TR>";
            }
        }
