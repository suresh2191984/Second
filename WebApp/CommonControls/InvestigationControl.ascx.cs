...
public partial class CommonControls_InvestigationControl : BaseControl
{
    public CommonControls_InvestigationControl()
        : base("CommonControls_InvestigationControl_ascx")
    {
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //onkeypress="javascript:setItem(event,this, '<%= ishdnID %>');" 
        ...
        listPKG.Attributes.Add("ondblClick", "javascript:onClick1('" + listPKG.ClientID + "','" + ishdnID + "');");
        listPKG.Attributes.Add("onkeypress", "javascript:setItem(event,this, '" + ishdnID + "');");
    ...
    public void LoadDatas(List<InvGroupMaster> lstGroups, List<InvestigationMaster> lstInvestigations)
    {
        ...
        if (lstInvestigations != null && lstInvestigations.Count > 0)
        { 
                txtINV.Visible = true;
                btnInvAdd.Visible = true;
                listINV.Visible = true;
                lblInvestigation.Visible = true;
                if (listINV.Items.Count > 0)
                {
                    listINV.Items.Clear();
                }

           // var l = lstInvestigations.FindAll(O => O.InvestigationID > 5932);
    ...
    public void LoadLabData(List<PatientInvestigation> lstGroups, List<PatientInvestigation> lstInvestigations)
    {
        if (IsCorporateOrg == "Y")
        {
            ...
            var Package = from groupName in lstGroups
                         where groupName.Type == "PKG"
                         select groupName;
    ...
    public void LoadLabData(List<PatientInvestigation> lstGroups, List<PatientInvestigation> lstPKG, List<PatientInvestigation> lstInvestigations, List<LabConsumables> lstLabConsumables, List<PatientInvestigation> lstPhyRates)
    {
       ...
       if (lstInvestigations != null && lstInvestigations.Count > 0)
        ...
        foreach (PatientInvestigation pInves in lstPhyRates)
        {
            ...
            if (FoundInv.Count() > 0)
            {
                HdnLPF.Value += pInves.InvestigationID + "~" + FoundInv[0].InvestigationName + "~ -" + CurrencyName + ": " + pInves.Rate + "~" + pInves.Type + "~" + pInves.GroupComment + "^";
            }
        }
    }
    ...
    public List<PatientInvestigation> GetOrderedList()
    {
        ...
        string strInvNameCmd = string.Empty;
            ...
            if (splitString != string.Empty)
            {
                ...
                if (lineItems.Length > 3)
                {
                    ...
                    if (lineItems[1].Contains("-"+CurrencyName+":"))
                    {
                        ...
                        strInvName = lineItems[1].Substring(0, rIndex);
                        strRate = lineItems[1].Substring(rIndex + 4);
                        //strInvNameCmd = lineItems[2].Substring(0, rIndex);
                        decimal.TryParse(strRate, out rate);
                        string type = lineItems[2];                        
                        PatientInves.Rate = rate;
                        PatientInves.Type = type;
                        //PatientInves.GroupComment = Xray;
                        if (type.ToUpper() == "INV")
                        {
                            PatientInves.InvestigationID = id;
                            PatientInves.InvestigationName = strInvName;
    ...
    public void setOrderedList(string List)
    {
        lblTotal.Text = "0";
        iconHid.Value = List;
        hdnOrderdINV.Value = List;
        if (IsCorporateOrg == "Y")
        {
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "tKey1", "javascript:LoadOrdItemsForHindu('');", true);
        }
        else
        {
    ...
    //            txtGroup.Text = "";
    //            iconHid.Value = orderedInv;
                //            ScriptManager.RegisterStartupScript(Page, this.GetType(), "tKey2", "javascript:alert('Investigation already added!');", true);
    ...
    public List<OrderedInvestigations> GetNewOrderedListForHindu()
    {
        List<OrderedInvestigations> lstpatInves = new List<OrderedInvestigations>();
        OrderedInvestigations PatientInves;
        long id = 0;
        string hidValue = hdnOrderdINV.Value;
        string hidValue1 = hdnOrderdINVNew.Value;
        string Drname = string.Empty;
        string Date = string.Empty;
        string iqty = string.Empty;
        string iAmt = string.Empty;
        ...
        decimal rate = 0;
        //"231~Platelet Rich Concentrate -Rs: 220.00~BLB^1008~Toxoplasma Virus - IgG -Rs: 0.00~INV^"
        foreach (string splitString in hidValue1.Split('^'))
        {
            if (splitString != string.Empty)
            {
                string[] lineItems = splitString.Split('~');
    ...
    public void loadOrderedList(List<OrderedInvestigations> lstInv, List<OrderedInvestigations> lstGrp)
    {
        ...
        {
            orderedInv += objOrderedGrp.ID + "~" + objOrderedGrp.Name + "~GRP" + "~" + objOrderedGrp.RefPhyName+ "^";
        }

        foreach (OrderedInvestigations objOrderedInv in lstInv)
        {
            orderedInv += objOrderedInv.ID + "~" + objOrderedInv.Name + "~INV" + "~" + objOrderedInv.RefPhyName + "^";
    ...
    public void LoadScheduleControl(string sPage)
    {
        hdnsPage.Value = sPage;
    }
 public string IsMappingtems
    {
        get { return hdnIsMappingtems.Value; }
        set { hdnIsMappingtems.Value = value; }
    }
}  
