...
public partial class CommonControls_Anjanabillprint : BaseControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string LOGOOriginalSize = string.Empty;
        LOGOOriginalSize = GetConfigValue("LogoWidthIncrease_Anj", OrgID); 
        string BillVisitLableChanges = string.Empty;
        BillVisitLableChanges = GetConfigValue("BillVisitLableChanges", OrgID);
        if (!Page.IsPostBack)
        {
            ...
            if (lstConfig.Count > 0)
            {
                ...
                if (lstConfig[0].ConfigValue.Trim() != "")
                {
                    if (Logohide == "Y")
                    {
                        lblTitleText.Style.Add("font-size", "xx-small"); 
                        tdbillBarcode.Style.Add("text-align", "center"); 
                        imgBillLogo.Visible = false;
                        imglogo2.Visible = false;
                        tdline1.Visible = false;
                        tblLine2.Visible = false; 
                    }
                    else
                    {
                    if (Logoright == "Y")
                    ...
                    else
                    {
                        ...
                        if (LOGOOriginalSize == "Y")
                        {
                            imgBillLogo.Width = 300;
                            gvBillingDetail1.Columns[4].Visible = false;
                        }
                        }
                    }
                }
            ...
            if (BillVisitLableChanges == "Y")
            {
                Rs_BillNo0.Text = "Transaction Number";
                Rs_VisitNumber.Text = "Bill / Visit Number";
            }
    ...
    public void BillPrinting(long visitID, long FinalbillID, int pdp)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(lstBillingDetail[0].Remarks))
            {
                lblRemark.Visible = true;
                lblRemark.Text = "Remarks: " + lstBillingDetail[0].Remarks.ToString();
            }
            else
            {
            ...
            if (lstBillingDetail.Count > 0 && lstFinalBillDetail[0].GrossBillValue > 0)
            {
                ...
                if (!string.IsNullOrEmpty(lstPatientDetails[0].Password) && !string.IsNullOrEmpty(lstPatientDetails[0].LoginName))
                {
                    tdUserName.Visible = true;
                    tbPassword.Visible = true;
                    tburl.Visible=true;
                    lblLoginName.Text = lstPatientDetails[0].LoginName;
             ...
             //   lblModetype.Text = lstPatientDetails[0].DispatchType.ToString();
                ...
                if (lblCurrentVisitDueText.Text != "")
                {
                    if (Convert.ToDouble(lblCurrentVisitDueText.Text) > 0)
                    {
                        ...
                        if (isDueBill == "Y")
                        {
                            trDue.Attributes.Add("visibility", "visible");
                        }
                    }
                    //if (lblCurrentVisitDueText.Text == "")
                    //{
                    ...
                    //    trDue.Attributes.Add("visibility", "hidden");
                    //}
                }
                else
                {
                ...
                //txtdiscountremarks.Text = lstFinalBillDetail[0].TPARemarks.ToString();
                lbldiscountremarks.Text = "Remarks: " + lstFinalBillDetail[0].TPARemarks.ToString();
                txtdiscountreason.Text = lstFinalBillDetail[0].DiscountReason.ToString();

                string ShowDiscountReasonAndRemarks = string.Empty;
                ShowDiscountReasonAndRemarks = GetConfigValue("ShowDiscountReasonAndRemarks", OrgID);
                if (ShowDiscountReasonAndRemarks == "Y")
                {
                    if (Convert.ToString(lstFinalBillDetail[0].TPARemarks) != "" && Convert.ToString(lstFinalBillDetail[0].TPARemarks) != null)
                    { 
                        trdiscountremarks.Visible = true;
                    }
                    if (txtdiscountreason.Text != "")
                    {
                        trdiscountreason.Visible = true;
                    }
                }
	      ...
	      lblTpaName1.Text = lstPatientDetails[0].TPAName;
              ...
              //  //if (lblTpaName1.Text == "GENERAL")
                ...
                string AmountReceivedHide = string.Empty;
                AmountReceivedHide = GetConfigValue("AmountReceivedHide", OrgID);
                if (!String.IsNullOrEmpty(AmountReceivedHide) && ISCash == "N")
                {
                    trAmountReceived.Visible = false;
    ...
    protected void gvBillingDetail1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            Label lblFee = (Label)e.Row.FindControl("lblDescription");

            string FeeDesc = lblFee.Text;
            if (FeeDesc.Contains("#"))
            {
                string[] Feeheader = FeeDesc.Split('#');
                string[] Feecontents = Feeheader[1].Split('~');
                int strcount = Feecontents.Count();
                string Finalvar = string.Empty;
                for (int i = 0; i < strcount; i++)
                {

                    Finalvar += "<BR/>" + Feecontents[i];

                }

                lblFee.Text = "<b>" + Feeheader[0] + "</b>" + Finalvar;
            }
        }
