...
public partial class CommonControls_IGeneticBillPrint : BaseControl
{
    ...
    public void BillPrinting(long visitID, long FinalbillID, int pdp)
    {
        try
        {
            ...
            if (lstBillingDetail.Count > 0 && lstFinalBillDetail[0].GrossBillValue > 0)
            {
                ...
                {
                    tdHis.Visible = true;                 
                    lblHistory.Style.Add("display", "block");
                    lblpatientHistory.Style.Add("display", "block");
                ...
                if (lstFinalBillDetail[0].TATDate > Convert.ToDateTime("01/01/1900"))
                {
                    ...
                    lblReportcommitDate.Text = "Please have your report collected at : " + "<b>" + lstFinalBillDetail[0].TATDate.ToString("dd/MMM/yyyy hh:mm:tt");
                ...
                if (!string.IsNullOrEmpty(lstPatientDetails[0].Password))
                {
                    ...
                    string lblPassword = string.Empty;
                    ...
                    if (!string.IsNullOrEmpty(lstPatientDetails[0].Password))
                    {
                        ...
                        lblPassword = DecryptedPassword;
                    }
                    string sPath = string.Empty;
                    sPath = Request.Url.ToString().Replace(Request.AppRelativeCurrentExecutionFilePath.Remove(0, 1).ToString(), "");
                    sPath = sPath.Split('?')[0] + "/" + "Home.aspx";
                    PatientPortal = "<b>" + "*" + "</b>" + "You can view your reports in following link, <u><hyperlink>" + sPath + "</hyperlink></u> . Your username is " + lstPatientDetails[0].PatientNumber.ToString() + " and Password is " + lblPassword.ToString() + "" + "<br>";
                    lblpatientportal.Text = PatientPortal.ToString();
                }
                ...
                if (patient_port == "Y")
                {
                    lblpatientportal.Visible = false;
                }
                //Comment by Prabakar//
                ...
                if (lstPatientDetails[0].TPAName == "GENERAL" || ClientTypeCode == "WAK" || ISCash == "Y")
                {
                    ...
                    TRAMTRcvd.Style.Add("display", "table-row");
                }
                else
                {
                ...
                if (IsLogoNeeded == "Y")
                {
                    //if (RoleHelper.CreditControler == RoleName || RoleHelper.Admin == RoleName || RoleHelper.SrCreditControler == RoleName
                    //    || RoleHelper == RoleName)
                    //{
                        if (lstPatientDetails[0].URNTypeId != null)
                        {
                            GetClientLogoImage(lstPatientDetails[0].URNTypeId);
                       // }
    ...
    public void BillPrinting1(long visitID, long FinalbillID, int pdp)
    {
        try
        {
            ...
            if (Request.QueryString["isFB"] != null && Request.QueryString["isFB"].ToString() == "Y")
            {
                ...
                BillingEngine billingBL = new BillingEngine(base.ContextInfo);
            billingBL.GetBillPrintingDetails_MRPBill(visitID, out lstBillingDetail,
                                            out lstFinalBillDetail, out lstPatientDetails,
                                            out lstOrganization, out physicianName,
                                            out lstDuePaidDetails, FinalbillID, out lstTax, out splitstatus, out lstQualification);
            hdnClientDuplicate.Value = lstPatientDetails[0].BillType;
            ...
            if (lstBillingDetail.Count > 0 && lstFinalBillDetail[0].GrossBillValue > 0)
            {
                ...
                if (lstFinalBillDetail[0].DiscountAmount > 0)
                {
                    trDiscount1.Attributes.Add("display", "block");
                ...
                if (lstPatientDetails[0].TPAName != "" && lstPatientDetails[0].TPAName != null)
                {
                    ...
                    {
                        trFinalBillHeader1.Attributes.Add("style", "display:block");
                ...
                if (isDynamic == "Y" && lstFinalBillDetail != null && lstFinalBillDetail.Count > 0)
                {
                    ...
                    PrinterHelper.PrintOPAmountDetails(ref objprint, NetValue.ToString(), RecAmount, grossbill, paymode,
                        (lblTaxAmount.Text).ToString(), (lblDiscount.Text).ToString(),
                        (lblRoundOff.Text).ToString(), (lblServiceCharge.Text).ToString(),
                    discountpercent, lblCurrentVisitDueText.Text, lblNetValue.Text, AmtResceived1,
                    PayingCurrency, pUser, Dueamounttxt, Dueamt, RemainDepo, depamt.ToString());
    ...
    public string getprinter()
    {
	 ...
	 public void GetClientLogoImage(long ClientId )
    {
       
        long returnCode = -1;
       //  ClientId = Convert.ToInt64(hdnId.Value);
        ...
        if (byteArray != null && byteArray.Count() > 0)
        {
            ...
            ClientLogo.Src = "../Admin/ClientLogoHandler.ashx?OrgID=" + OrgID + "&ClientId=" + ClientId;
            //ClientLogo.Style.Add("display", "block");
            tdClientLogo.Attributes.Add("Style", "block");
        }
        else
        {
    ...
    protected void gvBillingDetailD1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        GateWay gateWay = new GateWay();
        List<Config> lstConfig = new List<Config>();
        long returnCode = gateWay.GetConfigDetails("Amount(Rs)", OrgID, out lstConfig);
