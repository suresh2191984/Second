...
public partial class CommonControls_TravelHistory : BaseControl 
{
    ...
    public void EditData(List<PatientHistoryAttribute> lstPHA, long ID)
    {
        ...
        for (int j = 0; j < lstEditData.Count(); j++)
        {
            ...
            {
                rdoYes_1105.Checked = true;
                divrdoYes_1105.Style.Add("display", "block");
                txtTravel.Text = lstEditData[j].AttributeValueName;
            }
        }
    }
    public long GetData(out List<PatientHistory> attribute, out List<PatientHistoryAttribute> attrvalue)
    {
        ...
        if (rdoYes_1105.Checked == true)
        {
            PatientHistory objpatienthistory = new PatientHistory();
            objpatienthistory.HistoryID = Convert.ToInt32(1105);
            objpatienthistory.HistoryName = rdoYes_1105.Text;
            ...
            if (txtTravel.Text != "")
            {
                objpatienthistoryattribute.HistoryID = Convert.ToInt64(1105);
                objpatienthistoryattribute.AttributeID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributevalueID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributeValueName = txtTravel.Text;
