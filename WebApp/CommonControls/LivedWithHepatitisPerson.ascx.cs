...
public partial class CommonControls_LivedWithHepatitisPerson : BaseControl 
{
    ...
    public void EditData(List<PatientHistoryAttribute> lstPHA, long ID)
    {
        ...
        for (int j = 0; j < lstEditData.Count(); j++)
        {
            ...
            {
                rdoYes_1110.Checked = true;
                divrdoYes_1110.Style.Add("display", "block");
                txtLivedWithHepatitisPerson.Text = lstEditData[j].AttributeValueName;
            }
        }
    }
    public long GetData(out List<PatientHistory> attribute, out List<PatientHistoryAttribute> attrvalue)
    {
        ...
        if (rdoYes_1110.Checked == true)
        {
            PatientHistory objpatienthistory = new PatientHistory();
            objpatienthistory.HistoryID = Convert.ToInt32(1110);
            objpatienthistory.HistoryName = rdoYes_1110.Text;
            ...
            if (txtLivedWithHepatitisPerson.Text != "")
            {
                objpatienthistoryattribute.HistoryID = Convert.ToInt64(1110);
                objpatienthistoryattribute.AttributeID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributevalueID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributeValueName = txtLivedWithHepatitisPerson.Text;
