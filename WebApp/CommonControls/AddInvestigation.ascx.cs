...
public partial class CommonControls_AddInvestigation : BaseControl
{
    long returnCode = -1;
    ...
    int totalRows = 0;
    int totalpage = 0;
    public CommonControls_AddInvestigation()
        : base("CommonControls_AddInvestigation_ascx")
    {
    }
    ...
    public void LoadGrid()
    {
        try
        {
            ...
            int PageSize = 10;
            Investigation_BL invBL = new Investigation_BL(base.ContextInfo);
            returnCode = invBL.SearchInvestigation(OrgID, InvName, out lstInvestigation, currentPageNo, PageSize, out totalRows);
            if (lstInvestigation.Count > 0)
            {
                GrdFooter.Style.Add("display", "table-row");
                totalpage = totalRows;
