...
public partial class CommonControls_TravelToMalarialPlace : BaseControl 
{
    ...
    public void EditData(List<PatientHistoryAttribute> lstPHA, long ID)
    {
        ...
        for (int j = 0; j < lstEditData.Count(); j++)
        {
            ...
            {
                rdoYes_1109.Checked = true;
                divrdoYes_1109.Style.Add("display", "block");
                txtTravelToMalarialPlace.Text = lstEditData[j].AttributeValueName;
            }
        }
    }
    public long GetData(out List<PatientHistory> attribute, out List<PatientHistoryAttribute> attrvalue)
    {
        ...
        if (rdoYes_1109.Checked == true)
        {
            PatientHistory objpatienthistory = new PatientHistory();
            objpatienthistory.HistoryID = Convert.ToInt32(1109);
            objpatienthistory.HistoryName = rdoYes_1109.Text;
            ...
            if (txtTravelToMalarialPlace.Text != "")
            {
                objpatienthistoryattribute.HistoryID = Convert.ToInt64(1109);
                objpatienthistoryattribute.AttributeID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributevalueID = Convert.ToInt32(0);
                objpatienthistoryattribute.AttributeValueName = txtTravelToMalarialPlace.Text;
