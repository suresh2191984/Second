...
public partial class Investigation_aPTTPattern : BaseControl
{
    public Investigation_aPTTPattern()
        : base("Investigation_aPTTPattern_ascx")
    {
    }
 ...
 public long AccessionNumber
    ...
    public bool Readonly
    {
        set
        {
            //Added by Perumal on 23 Jan 2012
    ...
    public void LoadDataForEdit(List<InvestigationValues> lstValues)
    {
        ...
        //PTT(Activated Partial Thromboplastin Time)-Control aPTT
        for (int i = 0; i < lstValues.Count; i++)
        {
            if (lstValues[i].Name.Contains("-Control aPTT"))
            {
                txtControl.Text = lstValues[i].Value != string.Empty ? lstValues[i].Value : " ";
            }
            else if (lstValues[i].Name.Contains("-Patient aPTT"))
            {
                txtPatient.Text = lstValues[i].Value != string.Empty ? lstValues[i].Value : " ";
            }
        }
        txtReason.Text = lstValues[0].Reason;
        txtMedRemarks.Text = lstValues[0].MedicalRemarks; //Inv Remarks
    }
