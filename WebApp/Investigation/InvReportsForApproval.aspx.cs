...
public partial class Investigation_InvReportsForApproval : BasePage
{
    public Investigation_InvReportsForApproval()
        : base("Investigation_InvReportsForApproval_aspx")
    {
    }
    ...
    {
        ObjActionMaster = new ActionManager(base.ContextInfo);
        ...
        string ShowCancelBtn = GetConfigValues("DisableResultCaptureContinueButton", OrgID);
        if (ShowCancelBtn == "Y")
        {
        ...
        if (!IsPostBack)
        {
            ...
            hdnPrevPageURL.Value = null;
            hdnPrevPageURL.Value = Request.UrlReferrer.ToString();
            ReportViewer.Visible = false;
            Patient_BL objPatientBL = new Patient_BL(base.ContextInfo);
            ...
            if (lstReport.Count() > 0)
            {
                ...
                if (ShowSummaryReport == "Y")
                {
                    ...
                    Int32 RecordCount = 0;
                    bool ShowTRF = false;
                    objInvBL.CheckInvSummaryReport(OrgID, patientVisitID, out lstInvReportMaster, out RecordCount, out ShowTRF);

                    if (RecordCount > 0 && lstInvReportMaster.Count > 0)
                    {
                        InvReportMaster oInvReportMaster = lstInvReportMaster[0];

                        lnkSummaryReport.Visible = true;
                        hdnSummaryReportPath.Value = oInvReportMaster.ReportTemplateName;

                        if (ShowTRF)
                        {
                            ViewTRF.Visible = true;
                            TRFUC.ShowPhoto = false;
                        }
                    }
                    btnChangeSummary.Visible = true;
                }
                hdnInvId.Value = string.Join(",", ((from r in lstReport
            ...
            if (Request.QueryString["ShowSummaryReport"] != null && Request.QueryString["ShowSummaryReport"] == "false")
            {
                lnkSummaryReport.Visible = false;
                btnChangeSummary.Visible = false;
            }
        }
    ...
    protected void btnChangeSummary_Click(object sender, EventArgs e)
    {
        try
        {
            System.Collections.Specialized.NameValueCollection lstQueryString = Request.QueryString;
            ...
            Response.Redirect("~/Investigation/SummaryReport.aspx?" + queryString);
        }
        catch (Exception ex)
        {
            //  ErrorDisplay1.ShowError = true;
            //   ErrorDisplay1.Status = ex.Message;
            CLogger.LogError("Error while change summary report button click event", ex);
        }
    }
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        Response.Redirect(hdnPrevPageURL.Value);
    }
        ...
        if (e.CommandName == "ShowReport")
        {
            ...
            foreach (DataListItem dt in ctr.Items)
            {
                ...
                foreach (DataListItem chk in ctr1.Items)
                {
                    ...
                    Label lblStatus = (Label)chk.FindControl("lblStatus");
                    if (lblStatus != null && lblStatus.Text == "Paid")
                    {
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            patientVisitID = Convert.ToInt64(Request.QueryString["vid"]);
            gUID = Request.QueryString["gUID"].ToString();
            InvIDs = Request.QueryString["Invid"].ToString();
            long Pid = Convert.ToInt64(Request.QueryString["pid"]);
            string LabNo = Request.QueryString["LNo"].ToString();
            ...
            // Response.Redirect(Request.ApplicationPath + "/Investigation/InvestigationResultsCapture.aspx?vid=" + patientVisitID + "&gUID=" + gUID + "&Invid=" + InvIDs);
        }
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //Response.Redirect(Request.ApplicationPath + "/Investigation/MethodKitCapture.aspx?vid=" + patientVisitID + "&dID=" + deptID);
    ...
    protected void lnkSummaryReport_Click(object sender, EventArgs e)
    {
        ...
        {
            patientVisitID = Convert.ToInt64(patientVisitID);
            InvIDs = String.IsNullOrEmpty(hdnInvId.Value) ? "0" : hdnInvId.Value;
            ...
            ReportViewer.ServerReport.ReportPath = hdnSummaryReportPath.Value;
            ...
            Microsoft.Reporting.WebForms.ReportParameter[] reportParameterList = new Microsoft.Reporting.WebForms.ReportParameter[4];
            reportParameterList[0] = new Microsoft.Reporting.WebForms.ReportParameter("pVisitID", Convert.ToString(patientVisitID));
            reportParameterList[1] = new Microsoft.Reporting.WebForms.ReportParameter("OrgID", Convert.ToString(OrgID));
            reportParameterList[2] = new Microsoft.Reporting.WebForms.ReportParameter("InvestigationID", Convert.ToString(InvIDs));
        ...
        catch (Exception ex)
        {
            //ErrorDisplay1.ShowError = true;
            //   ErrorDisplay1.Status = ex.Message;
            CLogger.LogError("Error while generating summary report", ex);
        }
    }
    #endregion
    ...
    public void ShowReport(string reportPath, long visitID, string templateID, string InvID)
    {
        try
        {
            ...
            //byte[] bytes = ReportViewer.ServerReport.Render("PDF", deviceInfo, out mimeType, out encoding, out extension, out streamids, out warnings);
            ////FileStream fs = new FileStream(@"C:\Sample.PDF", FileMode.Create);
            ////fs.Write(bytes, 0, bytes.Length);
            ////fs.Close();
            ////code added for PDF test ends
        }
        catch (Exception ex)
        {
