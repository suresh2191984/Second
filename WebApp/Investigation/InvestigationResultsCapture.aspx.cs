...
public partial class Investigation_InvestigationResultsCapture : BasePage, System.Web.UI.ICallbackEventHandler
{
    public Investigation_InvestigationResultsCapture()
        : base("Investigation_InvestigationResultsCapture_aspx")
    {
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            InvestigationBL.GetInvestigatonResultsCapture(vid, OrgID, RoleID, gUID, DeptID, InvIDs, ILocationID, objLoginDetail, taskID, IsTrustedDetails, status, out lstOrdered, out header, out lRoleDeptMap, out LstmedRemarksRule);

            //Seetha Medical remarks

            string JsonOut = "";
            System.Web.Script.Serialization.JavaScriptSerializer serializer = new System.Web.Script.Serialization.JavaScriptSerializer();
            JsonOut = new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(LstmedRemarksRule);
            ...
            if (!IsPostBack)
            {
                ...
                {                    
                    ...
                    {
                        if (visitList[0].SpecialityID == 0)
                        {
                            new BarcodeHelper().SaveReportBarcode(visitList[0].PatientVisitId, OrgID, visitList[0].VisitNumber, "PVN");
                        }
                    }
                    else
                    {
                        long PatientVisitId = 0;
                        for (int j = 0; j < visitList.Count; j++)
                        {
                            if (PatientVisitId != visitList[0].PatientVisitId)
                            {
								if (visitList[0].SpecialityID == 0)
								{
									new BarcodeHelper().SaveReportBarcode(visitList[0].PatientVisitId, OrgID, visitList[0].VisitNumber, "PVN");
                            ...
                            PatientVisitId = visitList[0].PatientVisitId;
                        }
                    }
                }
                //
                ...
                if (nonAutoautherizedCount == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (returnStatus > 0)
                        {
                            ...
                            long res = -1;
                            //res = AM.PerformingNextStepNotification(PC, "", "");SELVA//14122020
                        }
                    }
                }
                else
    ...
    public Control loadBioControl(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                //Seetha Auto Remarks
                ScriptManager.RegisterStartupScript(this, this.GetType(), "CalculateMedicalRemarks" + bioPattern1.TxtControlID, "CalculateMedicalRemarks('" + bioPattern1.TxtControlID + "');", true);
    ...
    public Control LoadResultsFishPattern1(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            //FishResults1.BarCodeNumber = lstInve.BarcodeNumber;
    ...
    public Control LoadBioPattern2(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                TextBox txtResult = (TextBox)Bio1.FindControl("txtResult");
...
if (ddlstatus.Items.FindByValue("Completed_2") != null)
    ...
    private long AutoSave()
    {
        ...
        try
        {
            ...
            foreach (var O in lstPatientInv)
            {
                // VEL | AutoCertification set default Y | 11-03-2020 | Start 
                string AutoCertification = GetConfigValues("IscheckAutoCertification", OrgID);
                if ((string.IsNullOrEmpty(AutoCertification)) || AutoCertification == "N")
                {
                    O.IsAutoCertification = "Y";
                }
                // VEL | AutoCertification set default Y | 11-03-2020 | END 
                ...
                ////////}
                //////////////karthick//////////////////
                else if (O.Status == "Completed" && (O.IsAbnormal == "N" || O.IsAbnormal == "") && O.AutoApproveLoginID > 0 && hdnIsExcludeAutoApproval.Value == "N" && O.IsAutoAuthorize == "Y" && O.IsAutoCertification == "Y")
                {
                    ...
                    if (AutoCertifywithQC == "Y" && O.QCData == "1")
                    {
                        if (O.IsAutoValidation != "Not Apporved" )
                        {
                            ...
                            if (O.DeltaLowerLimit != 0 && O.DeltaHigherLimit != 0)
                            {
                                ...
                                if (isNum)
                                {
                                    if (O.DeltaLowerLimit != 0 && val >= O.DeltaLowerLimit && val <= O.DeltaHigherLimit && (O.QCData == "1"))
                                    {
                                        O.Status = "Approve";
                                    }
                                    else
            ...
            if (Flag == true)
            {
                ...
                ////Added by Arivalagan.kk for Hide status investigationID//
                String NeedLabTechApproval = GetConfigValues("NeedLabTechApproval", OrgID);
                if (NeedLabTechApproval == "Y")
                {
                    returnCode = saveResults.SaveInvestigationResults(pSCMID, LstOfBio, lstPatientInv, lstPatientInvSampleResults, lstPatientInvSampleMapping, lPFiles, vid, OrgID, deptID, ApprovedBy, gUID, PageContextDetails, out returnStatus, lstFinalReflexPatientinvestigation, isFromDevice, LstPIAttr);
                }
                else
                {
                    returnCode = saveResults.SaveInvestigationResults(pSCMID, LstOfBio, lstPatientInv, lstPatientInvSampleResults, lstPatientInvSampleMapping, lPFiles, vid, OrgID, deptID, 0, gUID, PageContextDetails, out returnStatus, lstFinalReflexPatientinvestigation, isFromDevice, LstPIAttr);
                }
           ...
           /* int critical = 0;
            critical=lstPatientInv.FindAll(p => p.IsSensitive == "Y").Count();
            if(critical>0)
            {*/
		   else{
                ActionManager AM = new ActionManager(base.ContextInfo);
                ...
                lstpagecontextkeys.Add(PC);
                long res = -1;
                //res = AM.PerformingNextStepNotification(PC, "", "");SELVA14122020
            }
    ...
    public static void AutoSaveHisto(List<List<InvestigationValues>> LstinvValues, List<PatientInvestigation> lstPatientInvs, long vid, string guid, int orgid)
    {
        try
        {
            ...
            returnCode = saveResults.SaveInvestigationResults(pSCMID, LstinvValues, lstPatientInvs, lstPatientInvSampleResults, lstPatientInvSampleMapping, lPFiles, vid, 0, deptID, orgid, guid, PageContextDetails, out returnStatus, lstFinalReflexPatientinvestigation, isFromDevice, lstPatAtt);
        }
        catch (Exception ex)
        {
