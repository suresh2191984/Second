...
public partial class Investigation_BatchwiseEnterresult : BasePage
{
    public Investigation_BatchwiseEnterresult()
        : base("Investigation_BatchwiseEnterresult_aspx")
    {
    }
...
////////////gopi/////
    public void loadQualitativeResultMaster()
    {
        ...
        returnCode = objInvBL.GetInvQualitativeResultMaster(out lstQualitativeResult);
    ...
    public void LoadPatterns()
    {
        try
        {
            ...
            int Issearched = 0;
            if ((VisitIDs != "0" && VisitIDs != "0,0") || InvID != 0 || worklistid != 0 || deviceid != 0 || deptID != 0 || headerID != 0 || protocalID != 0 || BarcodeNumber != "0" || SearchClientID != 0)
            {
                Issearched = 1;
                objInvBL.GetBatchWiseInvestigationResultsCaptureFormat(VisitIDs, OrgID, BarcodeNumber, RoleID, deptID, InvID, InvType, objLoginDetail, "N", worklistid, deviceid, isAbnormalResult, headerID, protocalID, hdnActionName.Value, isMaster, recordCount, SearchClientID, out lstOrdered);
            }
...
//arun --get a flag for the visit have barcode entry in REPORTBARCODE table or not 
                    ...
                    for (int i = 0; i <lstOrdered.Count; i++)
                    {
                        ...
                        //arun --get a flag for the visit have barcode entry in REPORTBARCODE table or not 
                        if (PatientVisitId != lstOrdered[i].PatientVisitID)
                        {
                            if (lstOrdered[i].IsReportable == false)
                            {
                                string visitnumber = string.Empty;
								if (!string.IsNullOrEmpty(lstOrdered[i].VisitNumber))
                                {
									visitnumber = lstOrdered[i].VisitNumber.IndexOf(' ') >-1?
										lstOrdered[i].VisitNumber.Split(' ')[0] : lstOrdered[i].VisitNumber;
									new BarcodeHelper().SaveReportBarcode(lstOrdered[i].PatientVisitID, OrgID, visitnumber, "PVN");
								}
                            }
                        }
                        PatientVisitId = lstOrdered[i].PatientVisitID;
                        //
                    }
                }
                catch (Exception ex)
            ...
            try
            {
                ...
                if ((!string.IsNullOrEmpty(hdnInvValues.Value)) && (!string.IsNullOrEmpty(hdnPatInv.Value)))
                {
                    ...
                    foreach (var O in lstPatientInv)
                    {
                        ...
                        if (O.IsAutoAuthorize == "Y" && O.Status != "Pending")
                        {
                            O.Status = "Approve";
                        }
                    }
    ...
    private void search()
    {
        ...
        try
        {
            ...
            Int32.TryParse(hdnPageNo.Value, out pageNo);
            if (string.IsNullOrEmpty(txtSearchClientName.Text))
            {
                hdnSearchClientID.Value = "0";
            }
            Int32.TryParse(hdnSearchClientID.Value, out SearchClientID);
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(lstInve.ValidationText))
            {
                if (!hFormulaCollection.Contains(lstInve.AccessionNumber + "," + lstInve.GroupID))
                {
                    ...
                    {
                        hFormulaCollection.Add(lstInve.AccessionNumber + "," + lstInve.GroupID, " var index = 5; var CurrentAccNo = id.split('~')[index]; var accNo = '" + ClientID + "'; var AccNo1 = accNo.split('~')[index]; if(CurrentAccNo == AccNo1) {" + lstInve.ValidationText + "}");
                    }
                }
                ...
                if (Formula.Contains("[" + lstInve.InvestigationID + "]"))
                {
                    ...
                    if (Formula.Replace(" ", "").Contains("[" + lstInve.InvestigationID + "]="))
                    {
                        ...
                        if (lstInve.ResultValueType == "NTS")
                        {
                            ...
                            {
                                Formula = Formula + "var indexval = 5; var lstCurrentAccNo = id.split('~'); var accNo = '" + ClientID + "'; var lstAccNo = accNo.split('~'); if(lstCurrentAccNo[indexval] == lstAccNo[indexval] && document.getElementById('" + ClientID + "').value!=''){ValidateInterpretationRange(" + bioPattern1.PatternID + ",'" + txtValue.ClientID + "','','" + hdnXmlContent.ClientID + "','" + hdnPanicXmlContent.ClientID + "','" + bioPattern1.DecimalPlaces + "','" + txtIsAbnormal.ClientID + "','" + bioPattern1.AutoApproveLoginID + "','" + lblName.ClientID + "','" + lblUnit.ClientID + "','" + lstInve.Age + "','" + lstInve.Sex + "');setCompletedStatus('" + lstInve.GroupName + "','" + ClientID + "','" + lstInve.IsAutoValidate + "');formatResult('" + ClientID + "','" + bioPattern1.DecimalPlaces + "');ReplaceNumberWithCommas('" + ClientID + "');}";
                            }
                            else
                            {
                                Formula = Formula + "var indexval = 5; var lstCurrentAccNo = id.split('~'); var accNo = '" + ClientID + "'; var lstAccNo = accNo.split('~'); if(lstCurrentAccNo[indexval] == lstAccNo[indexval] && document.getElementById('" + ClientID + "').value!=''){setCompletedStatus('" + lstInve.GroupName + "','" + ClientID + "','" + lstInve.IsAutoValidate + "');formatResult('" + ClientID + "','" + bioPattern1.DecimalPlaces + "');ReplaceNumberWithCommas('" + ClientID + "');}";
                            }
                        }
                        else
                        {
                            ...
                            {
                                Formula = Formula + "var indexval = 5; var lstCurrentAccNo = id.split('~'); var accNo = '" + ClientID + "'; var lstAccNo = accNo.split('~'); if(lstCurrentAccNo[indexval] == lstAccNo[indexval] && document.getElementById('" + ClientID + "').value!=''){setCompletedStatus('" + lstInve.GroupName + "','" + ClientID + "','" + lstInve.IsAutoValidate + "');formatResult('" + ClientID + "','" + bioPattern1.DecimalPlaces + "');}";
                            }
                        }
