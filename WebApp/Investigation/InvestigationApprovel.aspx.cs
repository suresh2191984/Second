...
public partial class Investigation_InvestigationApprovel : BasePage, System.Web.UI.ICallbackEventHandler
{
    public Investigation_InvestigationApprovel()
        : base("Investigation_InvestigationApprovel_aspx")
    {
    }
	...
	static    List<PatientInvestigation> lstOrderedAccessionnumber = new List<PatientInvestigation>();
    ...
    List<OrderedInvestigations> lstOI = new List<OrderedInvestigations>();
    List<PatientInvestigation> lstPatientInvCopy = new List<PatientInvestigation>();
...
string strUnknownF = Resources.CommonControls_ClientDisplay.CommonControls_TestMaster_ascx_086 == null ? "UnKnown" : Resources.CommonControls_ClientDisplay.CommonControls_TestMaster_ascx_086;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Request.QueryString["POrgID"] != null)
            {
                ...
                hdnPatOrgID.Value = orgid.ToString();
            ...
            if (vid != 0)
            {
                ...
                //objPatientBL.GetReportTemplate(patientVisitID, Convert.ToInt32(hdnOrgID.Value), out lstReport, out lstReportName, out lstDpts);
            ...
            List<MedicalRemarksRuleMaster> lstmedrulemaster = new List<MedicalRemarksRuleMaster>();
            long DeptID = -1;
            ...
            InvestigationBL.GetInvestigatonResultsCapture(vid, OrgID, RoleID, gUID, DeptID, InvIDs, ILocationID, objLoginDetail, taskID, IsTrustedDetails, status, out lstOrdered, out lstheaders, out lRoleDeptMap, out lstmedrulemaster);
            //Seetha Medical remarks
            string JsonOut = "";
            //System.Web.Script.Serialization.JavaScriptSerializer serializerremarks = new System.Web.Script.Serialization.JavaScriptSerializer();
            JsonOut = new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(lstmedrulemaster);
            hdnMedicalRemarksList.Value = JsonOut;
            ...
            if (!IsPostBack)
            {
                lstOrderedAccessionnumber = null;
                lstOrderedAccessionnumber = lstOrdered;
            }

            if (lstOrderedAccessionnumber != null && lstOrderedAccessionnumber.Count > 0)
            {
               // lstOrdered = lstOrderedAccessionnumber;
            }
                    ...
                    if (returncode == 0)
                    {
                        ...
                        nonValidatedCount = (from IL in FilteredInvestigations
                                             ...
                                             && IL.Status != InvStatus.Rejected && IL.Status != "Rejected" && IL.Status != InvStatus.Retest && IL.Status != "InActive" && IL.Status != InvStatus.Notgiven
                                                 ...
                                                 //&&IL.Status!=InvStatus.ResultDeflag &&IL.Status!=InvStatus.ResultDeflags
                                             && IL.Status != "ResultDeflag" && IL.Status != "ResultDeflags"
                        ...
                        if (chkStatus != InvStatus.Pending)
                        {
                            ...
                            if (chkStatus == InvStatus.Validate && nonAutoautherizedCount == 0 && nonValidatedCount == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (returnStatus > 0)
                                    {
                                        ...
                                        if ((lstPatientInvCopy[0].IsSensitive == "Y" || lstOI[0].IsSensitive == "Y") && lstOI[0].Print_Task == InvStatus.Validate)
                                        {
                                            PC.Reason = "IsSensitive";
                                        }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    string NeedRefMobileno = string.Empty;
                                    NeedRefMobileno = GetConfigValues("NeedRefDoctor_MobileNo", OrgID);

                                    if (NeedRefMobileno == "Y")
                                    {
                                        if (RoleName == RoleHelper.Doctor)
                                        {
                                            if (lstRefPhy.Count > 0)
                                            {
                                                if (lstRefPhy[0].Mobile == "")
                                                {
                                                    //ScriptManager.RegisterStartupScript(Page, this.GetType(), "Alert_002", "javascript:ValidationWindow('Ref Physician mobile number is missing, please take necessary action!','Alert');", true);
                                                   // ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "ScriptKey", "javascript:alert('Ref Physician mobile number is missing, please take necessary action!');", true);
                            ...
                            else
                            {
                                ...
                                if (chkStatus != InvStatus.Pending)
                                {
                                    ...
                                    if (hdnSaveToDispatch.Value == "1")
                                    {
                                        ...
                                        role.Add(roleid);
                                        new Navigation().GetLandingPage(role, out path);
                                        string Alertmsg = "Ref Physician mobile number is missing, please take necessary action";
                                        if (path != string.Empty)
                                        {
                                            if (IsneedRefmobileno == "Y" && IsPanicRange == "Y")
                                            {
                                                ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect","alert('"+Alertmsg+"'); window.location='" + Request.ApplicationPath + path+"';" , true);

                                                
                                            }
                                            else
                                            {
                                                Response.Redirect(Request.ApplicationPath + path, false);
                                            }
                                        }
                                    }
                                    else
                ...
                switch (item.Range)
                {
                    case "EQ":
					  if (resultValue.Contains(','))
                        {
                            string[] lstresultvalue = resultValue.Split(',');
                            if (lstresultvalue.Length > 1)
                            {
                                resultValue = lstresultvalue[0].ToString();
                            }
                        } 
                        if (resultValue.ToLower().Trim() == item.ValueRange.ToLower().Trim())
                        {
                            objpinv.InvestigationID = item.ReflexInvestigationID;
                            objpinv.InstrumentID = item.InvestigationID;
                            result = true;
                        }
                        break;
                    case "NEQ":
    ...
    private long AutoSave()
    {
        ...
        List<PatientInvestigationAttributes> LstPIAttr = new List<PatientInvestigationAttributes>();
        ...
        try
        {
            //DemoBL.pGetpatientInvestigationForVisit(vid, Convert.ToInt32(hdnOrgID.Value), ILocationID, gUID, out _lstInvestigations);

            //if (_lstInvestigations.Count > 0)
            //{
            //    for (int i = 0; i < _lstInvestigations.Count; i++)
            //    {
            //        DemoBL.GetIsSensitiveTestForVisit(_lstInvestigations[i].VisitID, _lstInvestigations[i].InvestigationID, _lstInvestigations[i].OrgID, _lstInvestigations[i].AccessionNumber, _lstInvestigations[i].Status, _lstInvestigations[i].Type, out lstOI);
            //        if (lstOI[0].IsSensitive == "Y" && lstOI[0].Print_Task == InvStatus.Validate)
            //        {
            //            if (RoleName != RoleHelper.SrLabTech)
            //            {
            //                CreateSensitiveTestTask();
            //            }
            //        }
            //    }
            //}
            ...
            for (int i = 0; i < result.Count; i++)
            {
                switch (Convert.ToInt32(lstpatternID[i]))
                {
                    ...
                    case (Int32)TaskHelper.Pattern.BioPattern1:
                        ...
                        if (PINV1.MedicalRemarksID != 0)
                        {
                            PatientInvestigationAttributes PatAttr = new PatientInvestigationAttributes();
                            PatAttr.PatientVisitID = PINV1.PatientVisitID;


                            PatAttr.OrgID = OrgID;
                            PatAttr.InvestigationID = PINV1.InvestigationID;
                            PatAttr.AccessionNumber = PINV1.AccessionNumber;
                            PatAttr.MedicalRemarksID = PINV1.MedicalRemarksID;
                            LstPIAttr.Add(PatAttr);

                        }
                        lstPatientInv.Add(PINV1);
                        if (checkInvest.IsSensitive == "Y")
                        {
                            // Assign Task to Cheif Pathologist Role for Releasing Sensitive Result Report
                            if (RoleName != RoleHelper.SrLabTech && PINV1.Status == InvStatus.Approved)
                            {
                                CreateSensitiveTestTask();
                            } 
                            lstPatientInvCopy.Add(PINV1);
                        }
                        break;
                    ...
                    case (Int32)TaskHelper.Pattern.BioPattern3:
                        ...
                        if (BioPatten3.IsSensitive == "Y")
                        {
                            // Assign Task to Cheif Pathologist Role for Releasing Sensitive Result Report
                            if (RoleName != RoleHelper.SrLabTech && PINV3.Status == InvStatus.Approved)
                            {
                                CreateSensitiveTestTask();
                            }
                            lstPatientInvCopy.Add(PINV3);
                        }
                        break;
                    ...
                    case (Int32)TaskHelper.Pattern.HistoPathologyPatternLilavathi:
                        ...
                        lPFiles = histoPathologyPatternLilavathi.GetInvestigationFiles(vid, out Flag);
                        PINV66.GroupName = histoPathologyPatternLilavathi.GroupName.Replace(":", "");
                        PINV66.GroupID = histoPathologyPatternLilavathi.GroupID;
	...
	///////////////////
            ...
            List<InvestigationValues> olstInvValue = new List<InvestigationValues>();
            foreach (List<InvestigationValues> olist in LstOfBio)
            {
                foreach (InvestigationValues ob in olist)
                {
                    olstInvValue.Add(ob);
                }
            }

            var objStatus = from olst in olstInvValue
                            group olst by olst.Status into lstTemp
                            select new { status = lstTemp.Key, count = lstTemp.Count() };
            /// Future usage ony 
            var objInvID = from olst in olstInvValue
                           where olst.Status == InvStatus.Approved
                           group olst by olst.GroupID into lstTemp
                           select new { invID = lstTemp.Key, count = lstTemp.Count() };
            ////
            ...
            if (_lstInvestigations.Count > 0)
            {
                for (int i = 0; i < _lstInvestigations.Count; i++)
                {
                    ...
                    foreach (var x in objStatus)
                    {
                        if (x.status == InvStatus.Approved && lstOI[0].IsSensitive == "Y" && lstOI[0].Print_Task == InvStatus.Validate)
                        {
                            if (RoleName != RoleHelper.SrLabTech)
                            {
                                CreateSensitiveTestTask();
                ...
                else
                {
                    ...
                    if (hdnSynopticTestFlag.Value != "")
                    {
                       ...
                       // List<PatientInvestigationAttributes> lstpat = new List<PatientInvestigationAttributes>();
                        returnCode = saveResults.SaveInvestigationResults(pSCMID, LstOfBio, lstPatientInv, lstPatientInvSampleResults, lstPatientInvSampleMapping, lPFiles, vid, Convert.ToInt32(hdnOrgID.Value), deptID, LID, gUID, PageContextDetails, out returnStatus, lstFinalReflexPatientinvestigation, isFromDevice, LstPIAttr);
            ...
            if (hdnSynopticTestFlag.Value == "")
            {
                ...
                if (cont > 0)
                {
                    ...
                    PC.ButtonValue = "Save";
                    PC.ContextType = "CV";
                    if ((lstPatientInv[0].IsSensitive == "Y" || lstOI[0].IsSensitive == "Y") && lstOI[0].Print_Task == InvStatus.Validate)
                    {
                        PC.Reason = "IsSensitive";
                    }
                    lstpagecontextkeys.Add(PC);
    ...
    public Control loadBioControl(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            bioPattern1.BarCodeNumber = lstInve.BarcodeNumber;
    ...
    public Control LoadFishPattern1(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            FishPt.BarCodeNumber = lstInve.BarcodeNumber;
            long ReturnRes;
    ...
    public Control LoadBioPattern2(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            Bio1.BarCodeNumber = lstInve.BarcodeNumber;
    ...
    public Control LoadBioPattern3(PatientInvestigation lstInve)
    {
        ...
        try
        {
            ...
            Bio.BarCodeNumber = lstInve.BarcodeNumber;
    ...
    //Medical Remarks
    public void CreateSensitiveTestTask()
    {
        try
        {
            long returncode = -1; 
            ...
            List<PatientVisitDetails> lstPatientVisitDetails = new List<PatientVisitDetails>();
            returncode = new PatientVisit_BL(base.ContextInfo).GetVisitDetails(vid, out lstPatientVisitDetails);
            returncode = Utilities.GetHashTable(Convert.ToInt32(TaskHelper.TaskAction.Release),
                                                            vid, 0, PatientID, lstPatientVisitDetails[0].TitleName + " " +
                                                            lstPatientVisitDetails[0].PatientName, "", 0, "", 0, "", 0, "INV"
                                                            , out dText, out urlVal, "0", lstPatientVisitDetails[0].PatientNumber, 0,
                                                            gUID, lstPatientVisitDetails[0].ExternalVisitID, lstPatientVisitDetails[0].VisitNumber, "");
            task.TaskActionID = Convert.ToInt32(TaskHelper.TaskAction.Release);
            task.DispTextFiller = dText;
            task.URLFiller = urlVal;
            task.RoleID = RoleID;
            task.OrgID = Convert.ToInt32(hdnOrgID.Value);
        ...
        {
            CLogger.LogError("Error While Create Task For Sensitive Test", ex);
        }
    }
