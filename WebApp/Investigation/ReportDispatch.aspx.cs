...
public partial class Investigation_ReportDispatch : BasePage
{
    ...
    string strregs = Resources.Investigation_ClientDisplay.Investigation_VisitDetails_aspx_21 == null ? "REPORT CHECK LIST" : Resources.Investigation_ClientDisplay.Investigation_VisitDetails_aspx_21;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                string ln = new BasePage()._LoginName;
    ...
    public void PatientGetReportsPrint(int currentPageNo, int PageSize)
    {
        try
        {
            ...
            returnCode = new PatientVisit_BL(base.ContextInfo).pGetVisitSearchDetailbyPNoPrint("",
            ...
            lstDispatchDetails, currentPageNo, PageSize, out totalRows, "", IsPrintAll, PrintLocationID, IsColorPrint,IPOPNumber,PatientPayType,PatientStatus,"");

            excelgrid.Visible = true;
            excelgrid.DataSource = null;
            excelgrid.DataBind();
            excelgrid.DataSource = lstPatientVisit;
            excelgrid.DataBind();
            ...
            if (ddlPrintType.SelectedValue != "RPTCHK")
            {
                ...
                foreach (PatientVisit lst in lstPatientVisit)
                {
                    ...
                    if (ddlPrintType.SelectedValue == "BILL")
                    {
					 ...
					 //changes by arun - Save PDF added
                    if (ddlVisitActionName.SelectedValue == "Save_PDF")
                    {
                        PC.ActionType = "SAVEROUNDDB";
                        PC.ActionType = "SAVEROUNDDB";
                    }
    ...
    protected void dlstDispatchList_OnDataBinding(object sender, DataListItemEventArgs e)
    { 
    }
    public void PatientGetReports(int currentPageNo, int PageSize)
    {
        try
        {
            ...
            if ((RoleName != RoleHelper.Patient) && (RoleName != RoleHelper.Physician))
            {
                ...
                returnCode = new PatientVisit_BL(base.ContextInfo).pGetVisitSearchDetailbyPNoPrint(txtPatientNo,
                    txtName.Text.ToString(), txtMobile.Text, fromDate, ToDate, OrgID, lstTOD, pSearchType,
                ...
                PrintLocationID, IsColorPrint,IPOPNumber,PatientPayType,PatientStatus,"");
            ...
            if (ddlPrintType.SelectedValue == "RPTCHK")
            {
                ...
                List<PatientVisit> lstClientData = new List<PatientVisit>();
                List<string>  lstClient = lstPatientVisit.Select(a => a.ClientName).Distinct().ToList();
                StringBuilder sb = new StringBuilder();
                lblPrintedAt.Text = string.Empty;
                Labelorgname.Text = string.Empty;
                lblPrintedUser.Text = string.Empty;
                string orgName;
                ...
                lblPrintedAt.Text = "Checklist Printed on " + OrgDateTimeZone;
                lblPrintedUser.Text = "Printed User " + _LoginName;
                lblPrintedNoOfCount.Text = strTotCount + totalRows.ToString() + " ";

                
                
                foreach (string cname in lstClient)
                {
                    
                    
                    lstClientData = lstPatientVisit.Where(a => a.ClientName == cname).ToList();
                    int rno=1;

                    sb.Append("<table class='tblrpt'><thead><th>Sr.No</th><th>Request Number</th><th>Patient Name</th><th>Visit Number</th><th>Gender/Age</th><th>Client Name</th><th>Visit Datetime</th><th>Approved Test Description</th><th>Pending Test</th><th>Remarks</th></thead><tbody>");
                    foreach (PatientVisit pv in lstClientData)
                    {

                        sb.Append( "<tr><td>" + rno + "</td><td>" + pv.ExternalVisitID + "</td><td>" + pv.PatientName + "</td><td>" + pv.VisitNumber + "</td><td>" + getAge(pv.PatientAge) + "</td><td>" + pv.ClientName + "</td><td>" + pv.ICDCodeStatus + "</td><td>" + pv.PriorityName + "</td><td>" + pv.SpecialityName + "</td><td>" + pv.Remarks + "</td></tr> "); 
                        rno+=1;
                    }
                    sb.Append( "</tbody></table>");
                    
                }
                divrptchklst.InnerHtml = sb.ToString();
            ...
            if (lstPatientVisit.Count > 0)
            {
                ...
                if (lstActionMaster.Count > 0)
                {
                    ...
                    #endregion
                    //changes by arun - Save PDF Added
                    ddlVisitActionName.DataSource = lstActionMaster.FindAll(p => (p.ActionCode == "Show_Print" || p.ActionCode == "Save_PDF"));
    ...
    private string getAge(string PatientAge)
    {
        string age="";
    string[] splityears = PatientAge.Split(' ');
    string[] splityears1 = PatientAge.Split('/');
    ...
    else
    {
        if (splityears.Count() > 1)
        {
            if (splityears[1].ToString() == "Year(s)")
            {
                age = splityears[0].ToString() + " y";
            }

            if (splityears[1].ToString() == "Month(s)")
            {
                age = splityears[0].ToString() + " m";
            }
            if (splityears[1].ToString() == "Day(s)")
            {
                age= splityears[0].ToString() + " d";
            }
        }
        else
        {

            age = splityears[0].ToString() + " -";
        }
}
    return age;
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        ...
        {
			//changes by arun - Save PDF added
            if (ddlVisitActionName.SelectedValue == "Show_Print" || ddlVisitActionName.SelectedValue == "Save_PDF")
            {
                        ...
                        if (gvRow.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            if (chkSel.Checked == true)
                            {
                                ...
                                if (ddlPrintType.SelectedValue == "RPT")
                                {
                                    ...
                                    PC.ButtonValue = "REPORTPRINTBTN";
                                   // PC.Reason = "";
                                    PC.Reason = ddlPatientPaytype.SelectedValue;
                                }
                                if (ddlPrintType.SelectedValue == "BILL")
                                {
                                    PC.ButtonName = "BILLPRINTBTN";
    ...
    protected void ZoneDatList_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            Label Labelorgname = (Label)e.Item.FindControl("Labelorgname");
            Label lblPrintedUser = (Label)e.Item.FindControl("lblPrintedUser");
            Label lblPrintedNoOfCount = (Label)e.Item.FindControl("lblPrintedNoOfCount");
            if (txtFromPeriod.Text.ToString() == "")
            {
    ...
    protected void dlstDispatchList_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        ...
        foreach (DataListItem item in ((DataList)sender).Items)
        {
            string name = ((Label)item.FindControl("lblRowNumber")).Text;
        }
    ...
    protected void DispatchDatList_ItemDataBound(object sender, DataListItemEventArgs e)
    {
    }
    ...
    public void LoadMetaData()
    {
        ...
        {
            long returncode1 = -1;
            string domains1 = "PrintType,Printstatus,CustomPeriodRange,DespatchType,PatientPayType";
        ...
        protected void imgbtnxls_Click(object sender, ImageClickEventArgs e)
    {
       ...
       this.EnableViewState = false;
        ...
        //excelgrid.HeaderRow.Style.Add("background-color", "");
        //for (int i = 0; i < excelgrid.HeaderRow.Cells.Count; i++)
        //{
        //    excelgrid.HeaderRow.Cells[i].Style.Add("background-color", "");
        //}
