<%@ Control Language="C#" AutoEventWireup="true" CodeFile="TablePatternV2.ascx.cs" Inherits="Investigation_TablePatternV2" %>
        ...
        for (var r = 0; r < totalRowCount; r++) {
            ...
            $.each(ControlType, function(i, obj) {
                ...
                if (Type == "dropdown") {
                    ...
                    if (lstColumnBulkData.length > 0) {
                        ctrlVal = '<input id="ctl' + count + '" type="text" style="width:200px; height: 40px;" value=""  /><input id="hdn' + count + '" type="hidden" value="' + Type + '"/>';
                    }
                    else {
                        ctrlVal = '<input id="ctl' + count + '" type="text" style="width:200px; height: 40px;"  /><input id="hdn' + count + '" type="hidden" value="' + Type + '"/>';
                    }
                    bindCtrlVal = $('<td/>').html(ctrlVal);
                    row$.append(bindCtrlVal);
                    Type = "";
        ...
        if (document.getElementById(ctrlID + '_hdnBindData').value != "") {
            ...
            $.each(BindData, function(i, obj) {
                ...
                $.each(ControlType, function(k, obj1) {
                    ...
                    ctrlVal = '<input id="ctl' + count + '" type="text" style="width:200px; height: 40px;" value="' + value + '"/><input id="hdn' + count + '" type="hidden" value="' + Type + '"/> ';
                        bindCtrlVal = $('<td/>').html(ctrlVal);
                        row$.append(bindCtrlVal);
                        Type = "";
                        ctrlVal = "";
