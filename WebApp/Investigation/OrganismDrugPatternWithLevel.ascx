<%@ Control Language="C#" AutoEventWireup="true" CodeFile="OrganismDrugPatternWithLevel.ascx.cs"
    Inherits="Investigation_OrganismDrugPatternWithLevel" %>
...
<script language="javascript" type="text/javascript">
    ...
    function onAddNewDrugDetails(obj) {
        ...
        try {
            ...
            var tdddlLevelMap = row.insertCell(2);
            var selectElement = document.createElement("select");
            selectElement.id = "ddlLevelMap";
            selectElement.setAttribute("class", "ddl");
            var option1 = new Option(vSelect, "0");
            selectElement.options[selectElement.options.length] = option1;
            tdddlLevelMap.appendChild(selectElement);
            /* END | sabari | 20181129 | Dev | Culture Report */
            var tdZone = row.insertCell(3);
    ...
    function LoadXMLDrugList(organismName, resultXML, hdnInvID, hdnOrganismCount, divTblOrgans) {
        ...
        try {
            ...
            if (typeof (divElement) != 'undefined' && divElement != null) {
                ...
                /* BEGIN | sabari | 20181129 | Dev | Culture Report */
                var thLevelMap = $('<th/>').html(vLevelMap);
                /* END | sabari | 20181129 | Dev | Culture Report */
                var thZone = $('<th/>').html(vMICVal);
                var thAction = $('<th/>').html(vAction);
                headerRow$.append(thDrug).append(thSensitivity).append(thLevelMap).append(thZone).append(thAction);
		...
		var x = jQuery.noConflict();
                ...
                $(OrganXMLData).find('Organ').each(function() {
                    ...
                    /* BEGIN | sabari | 20181129 | Dev | Culture Report */
                    var DrugID = '';
                    if (typeof ($(this).attr('DrugID')) != 'undefined') {
                    ...
                    var LevelName = '';
                    if (typeof ($(this).attr('LevelName')) != 'undefined') {
                        LevelName = $(this).attr('LevelName');
                    }
                    var LevelID = '';
                    if (typeof ($(this).attr('LevelID')) != 'undefined') {
                    ...
                    if (typeof (family) != 'undefined' && family != familyName) {
                        ...
                        var tdFamilyName = $('<td colspan="5" align="center" style="font-weight:bold;"/>').html('<span id="spanFamily">' + family + '</span>');
                    ...
                    /* BEGIN | sabari | 20181129 | Dev | Culture Report */
                    var hdnDrugID = '<input type="hidden"  id="hdnDrugID" value="' + DrugID + '" />';
                    /* END | sabari | 20181129 | Dev | Culture Report */
                    var tdDrug = $('<td/>').html('<input type="text" id="txtDrugName" class="Txtboxsmall" value="' + DrugName + '" />' + hdnNameSeq + hdnFamilySeq + hdnDrugID);
                    ...
                    /* BEGIN | sabari | 20181129 | Dev | Culture Report */
                    var lstDrugLevelMapDetails = [];
                    if (DrugID != null && DrugID != "" && DrugID > 0) {
                        //var rsltlist = GetDrugLevelMapDetails(DrugID);
                        try {
                            $.ajax({
                                type: "POST",
                                url: "../WebService.asmx/GetOrganismDrugLevelMapDetails",
                                data: "{DrugID: " + DrugID + "}",
                                contentType: "application/json; charset=utf-8",
                                dataType: "json",
                                async: false,
                                success: function Success(data) {
                                    lstDrugLevelMapDetails = data.d;
                                },
                                error: function(xhr, ajaxOptions, thrownError) {
                                }
                            });
                    ...
                    var ddlLevelMap = $('<select id="ddlLevelMap" />');
                    ddlLevelMap.addClass('ddl');
                    $('<option />', { value: '0', text: vSelect }).appendTo(ddlLevelMap);
                    if (lstDrugLevelMapDetails != null && lstDrugLevelMapDetails.length > 0) {
                        //                        $.each(lstDrugLevelMapDetails, function(index, val) {
                        //                            ddlLevelMap.append($('<option></option>').val(val.LevelID).html(val.LevelName)
                        //                            );
                        //                        });
                        //sabari added temp
                        $.each(lstDrugLevelMapDetails, function(index, val) {
                            if (LevelID == val.LevelID) {
                                //$('<option></option>').val(val.LevelID).html(val.LevelName).selected: true.append(ddlLevelMap);
                                $('<option />', { value: val.LevelID, text: val.LevelName, selected: true }).appendTo(ddlLevelMap);
                            }
                            else {
                                //ddlLevelMap.append($('<option></option>').val(val.LevelID).html(val.LevelName));
                                $('<option />', { value: val.LevelID, text: val.LevelName }).appendTo(ddlLevelMap);
                            }
                        });
                        //end sabari added
                        ...
                        //ddlLevelMap.options.length = 0;
                        ddlLevelMap.empty();
                        var optn = document.createElement("option");
                        optn.text = vSelect; /*"--Select--";*/
                        optn.value = "0";
                        ddlLevelMap.append(optn);
                    }
                    /*Here we have set Existing select item value*/
                    ddlLevelMap.val(LevelName);
                    var tdddlLevelMap = $('<td/>').html(ddlLevelMap);
                    /* END | sabari | 20181129 | Dev | Culture Report */
                    var tdZone;
                    ...
                    row$.append(tdDrug).append(tdSensitivity).append(tdddlLevelMap).append(tdZone).append(tdDelete);
                    tbody$.append(row$);
                });
                table$.append(tbody$);
                $(divTag).html(table$);
        ...
        try {
            ...
            if (typeof (divElement) != 'undefined' && divElement != null) {
                ...
                $.each(lstOrganismDrugDetails, function(i, obj) {
                    ...
                    var lnkDelete = '<input id="btnDelete"  value=' + vdelete + ' type="button" style="background-color: Transparent; color: Blue; border-style: none; text-decoration: underline; cursor: pointer; font-size: 11px;" onclick="return onDeleteDrugDetails(this);" />';
                    var tdDelete = $('<td/>').html(lnkDelete);
                    /*Below Here we have append into table row */
                    row$.append(tdDrug).append(tdSensitivity).append(tdddlLevelMap).append(tdZone).append(tdDelete);
                    tbody$.append(row$);
                });
                table$.append(tbody$);
    ...
    /* BEGIN | sabari | 20181129 | Dev | Culture Report */
    function GetDrugLevelMapDetails(DrugID) {
        try {
            $.ajax({
                type: "POST",
                url: "../WebService.asmx/GetOrganismDrugLevelMapDetails",
                ...
                dataType: "json",
                success: function Success(data) {
                    var lstDrugLevelMapDetails = data.d;
                },
                error: function(xhr, ajaxOptions, thrownError) {
                }
            });
            ...
            $('div[id^="' + divColumnID + '"]').children().each(function() {
                ...
                $(this).children('tbody').children().each(function(i, n) {
                    ...
                    if ($row.children().length == 1) {
                        ...
                        /* BEGIN | sabari | 20181129 | Dev | Culture Report */
                        var _LevelID=$row.find($('select[id$="ddlLevelMap"] option:selected')).val();
                        var _LevelName=$row.find($('select[id$="ddlLevelMap"] option:selected')).text();
                        //var LevelID=$row.find($('#ddlLevelMap option:selected')).val();
                        //var LevelName=$row.find($('#ddlLevelMap option:selected')).text();
                         if (_LevelID == '0' ) {
                            _LevelID = 0;
                            _LevelName='';
                        }
                        var _DrugID = $row.find($('input[id$="hdnDrugID"]')).val();
                        /* END | sabari | 20181129 | Dev | Culture Report */
