...
public partial class Investigation_BulkEnterResult : BasePage
{
    public Investigation_BulkEnterResult()
        : base("Investigation_BulkEnterResult_aspx")
    {
    }
    ...
    string CollectlocationFilter = string.Empty;
    ...
    //string strNA = Resources.Investigation_ClientDisplay.Investigation_BatchWiseEnterresult_aspx_16 == null ? "NA" : Resources.Investigation_ClientDisplay.Investigation_BatchWiseEnterresult_aspx_16;
    string strNA = "NA";
    #endregion
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
        string strBulkComplete = "Bulk Complete" ;
        string strBulkValidate = "Bulk Validate" ;
        /* END | NA | Sabari | 20190508 | Created | BulkEnterResult */
        objInvBL = new Investigation_BL(base.ContextInfo);

        try
        {
            CollectlocationFilter = GetConfigValue("CollectlocationFilter", OrgID);
            if (CollectlocationFilter == "Y")
            {
                tdcolllocation.Style.Add("display", "table-cell");
                tdddlcolllocation.Style.Add("display", "table-cell");
            }
            else
            {
                tdcolllocation.Style.Add("display", "none");
                tdddlcolllocation.Style.Add("display", "none");
            }
            if (string.IsNullOrEmpty(hdnIsCancelLoad.Value) || hdnIsCancelLoad.Value != "1")
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
                    //if (RoleName == "Lab Technician" || RoleName == "SrLabTech")
                    //{                            // ddlUser.DataSource = lstDeptMaster;
                    //    lblUser.Visible = true;
                    //    ddlUser.Visible = true;
                    //    ddlUser.DataTextField = "Name";
                    //    ddlUser.DataValueField = "LoginID";
                    //    // ddlUser.DataBind();
                    //    ddlUser.Items.Insert(0, strSelect.Trim());
                ...
                /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
                #region BulkEnterResultRoleBasedActionNamestart
                //else if (Request.QueryString["Action"] == null)
                //{
                //    long returncode = -1;
                //    string domains = "BulkEnterResultRoleBasedActionName";
                ...
                //    {
                //        var BulkEnterResultRoleBasedActionNameValue = lstmetadataOutput
                //                                    .Where(p => p.Domain == "BulkEnterResultRoleBasedActionName" && p.Code == this.RoleName)
                //                                    .ToList();
                //        if (BulkEnterResultRoleBasedActionNameValue.Count > 0)
                //        {
                //            hdnActionName.Value = Convert.ToString(BulkEnterResultRoleBasedActionNameValue[0].DisplayText);
                //        }
                //    }
                //}
                /* END | NA | Sabari | 20190508 | Created | BulkEnterResult */
                ...
                if (hdnActionName.Value == "Validate")
                {
                    ...
                    lblPageHeader.Text = strBulkValidate.Trim();
                    hdnDefaultDropDownStatus.Value = "Validate";
                    Session["Action"] = hdnActionName.Value;
                }
                /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
                else if (hdnActionName.Value == "EnterResult")
                {
                    lblPageHeader.Text = strBulkComplete.Trim();
                    Session["Action"] = hdnActionName.Value;
                }
                /* END | NA | Sabari | 20190508 | Created | BulkEnterResult */
    ...
    public void LoadMetaData()
    {
        ...
        {
            long returncode = -1;
            //string domains = "Btc_Type";
            /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
            string domains = "Btc_Type,VisitType,DeltaCheck,QcCheck,BulkEnterResultRoleBasedActionName";
            /* END | NA | Sabari | 20190508 | Created | BulkEnterResult */
            string[] Tempdata = domains.Split(',');
    ...
    public void LoadPatterns()
    {
        try
        {
            ...
            Issearched = 1;
            /* BEGIN | NA | Sabari | 20190508 | Created | BulkEnterResult */
            if (hdnActionName.Value != "EnterResult" && hdnActionName.Value != "Validate")
            {
                if ((VisitIDs != "0" && VisitIDs != "0,0") || InvID != 0 || worklistid != 0 || deviceid != 0 || deptID != 0 || headerID != 0 || protocalID != 0 || BarcodeNumber != "0")
                {
                    objInvBL.GetBatchWiseInvestigationResultsCaptureFormat(VisitIDs, OrgID, BarcodeNumber, RoleID, deptID, InvID, InvType, objLoginDetail, "N", worklistid, deviceid, isAbnormalResult, headerID, protocalID, hdnActionName.Value, isMaster, recordCount,-1, out lstOrdered);
                }
            }
            else /* Bulk EnterResult Block*/
            {
                objInvBL.GetBatchWiseInvestigationResultsCaptureFormatBulkEnterResult(VisitIDs, OrgID, BarcodeNumber, RoleID, deptID, InvID, InvType, objLoginDetail, "N", worklistid, deviceid, isAbnormalResult, headerID, protocalID, hdnActionName.Value, isMaster, recordCount, Visittype, Location1, QcCheck, PatIds, CollectLocID, out lstOrdered);
            }
            /* END | NA | Sabari | 20190508 | Created | BulkEnterResult */
    ...
    private void search()
    {
        ...
        try
        {
            ...
            CollectLocID = ddlCollelocation.SelectedValue;
    ...
    public void LoadProcessingLocation()
    {
        long returnCode = 0;
        ...
        if (lstLocation.Count > 0)
        {
            ...
            ddlCollelocation.DataSource = lstLocation;
            ddlCollelocation.DataTextField = "Location";
            ddlCollelocation.DataValueField = "AddressID";
            ddlCollelocation.DataBind();



            ddlCollelocation.Items.Insert(0, strSelect.Trim());
            ddlCollelocation.Items[0].Value = "-1";
