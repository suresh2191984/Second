...
public partial class Investigation_InvestigationQuickApprovelCovid : BasePage
{
    public Investigation_InvestigationQuickApprovelCovid()
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    //hdnTaskID.Value = Request.QueryString["Covid"].ToString();
                }
    ...
    public void LoadGrid(int currentPageNo, int PageSize, bool fromEdit)
    {
        ...
        if (Request.QueryString["Covid"] != null)
        {
            Covid = Convert.ToString(Request.QueryString["Covid"]);
        }
        if (Covid == "Y")
        {
            Preference = "Negative";
    ...
    public long DoAction(long TaskID, long vid, string gUID, string pName, out List<InvOrgAuthorization> lstInvAuthorization, out int taskActionID)
    {
        ...
        if (cnt > 0 || cntPA > 0 || cntWH > 0 || cntRJ > 0)
        {
            if ((NonCoPublishTestCount > 0) || (IsResult == true))
            {
                ...
                PC.PageID =740;// Convert.ToInt64(PageContextDetails.PageID);
                PC.ButtonName ="btnApprove";// PageContextDetails.ButtonName;
                PC.ButtonValue = "Approve";// PageContextDetails.ButtonValue;
