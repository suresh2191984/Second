<%@ WebHandler Language="C#" Class="TRFImagehandler" %>
using System;
using System.Web;
using System.Net;
...
public class TRFImagehandler : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            string HCTechScheduler = GetConfigValue("IsHCSchedulerWorkflow", Convert.ToInt16(context.Request.QueryString["OrgID"]));
            CLogger.LogWarning("ProcessRequest Start");
            if (context.Request.QueryString["PictureName"] != null)
            {
		CLogger.LogWarning(context.Request.QueryString["PictureName"]);
                if (context.Request.QueryString["PictureName"] != "StationaryPdf")
                {
                    ...
                    CLogger.LogWarning("filePath -  " + filePath);
			 ...
			 CLogger.LogWarning("pathname  -  " + pathname );
                    ...
                    {
					CLogger.LogWarning("if (File.Exists(filePath))Start");
					CLogger.LogWarning("filePath" + filePath);
                ...
                {
				CLogger.LogWarning("if (File.Exists(filePath))Else");
