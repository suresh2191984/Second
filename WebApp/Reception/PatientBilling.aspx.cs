...
public partial class Reception_PatientBilling : BasePage
{
    long visitID = 0;
    long patientID = 0;
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["vid"] != null)
                {
                    ...
                    List<VisitClientMapping> lstPatientVisit = new List<VisitClientMapping>();

                    new PatientVisit_BL(base.ContextInfo).GetVisitClientMappingDetails(OrgID, visitID, out lstPatientVisit);

                    ddlClients.SelectedValue = lstPatientVisit[0].ClientID.ToString();
                    ddlCorporate.SelectedValue = lstPatientVisit[0].RateID.ToString();
                    ipConsultation.VisitRateID = (int)lstPatientVisit[0].RateID;
    ...
    protected void QuickBill_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect(@"../Billing/QuickBill.aspx?pid=" + patientID + "&vid=" + visitID + "&pname=" + hdnPName.Value + "&vType=OP", true);
        }
        catch (System.Threading.ThreadAbortException tae)
        {
        ...
        {
            CLogger.LogError("Error in Order Services - QuickBill_Click", ex);
        }
    }
