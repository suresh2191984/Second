<%@ Page Language="C#" AutoEventWireup="true" CodeFile="TestCancellation.aspx.cs"
    Inherits="Lab_TestCancellation" meta:resourcekey="PageResource1" %>
...
<head>
    ...
    <script type="text/javascript">
        function ValidationForRejection() {
            var InvSelect = SListForAppMsg.Get('Lab_InvestigationOrgChange_aspx_06') == null ? "Please Select Atleast a Investigation" : SListForAppMsg.Get('Lab_InvestigationOrgChange_aspx_06');
            var strRejection = SListForAppMsg.Get('Lab_InvestigationOrgChange_aspx_03') == null ? "Please Select All the Investigationa And Then Reject" : "Please Select All the Investigationa And Then Reject";
            var grdview = document.getElementById('grdResult');
            var Tcount = grdview.rows.length;
            var Childcount = 0;
            for (var i = 0; i < Tcount; i++) {
                var intputs = grdview.rows[i].getElementsByTagName('input');
                if (intputs[0].checked) {
                    Childcount++;
                }
            }
            if (Childcount != Tcount) {
                ValidationWindow(strRejection, AlertType);
                return false;
            }
        }
        ...
        function ValidateIfSampleReceivedOrNot() {
            var status = true;
            var chkboxes = $('#grdResult').find('tr').find('span[itemstatus="N"]');
            $.each(chkboxes, function(id, val) {
                if ($(val).find('input[type="checkbox"]').prop('checked')) {
                    var tname = $(val).closest('tr').find('td')[2];
                    tname = $(tname).find('span')[0];
                    // $(tname).find('span').innerHtml;
                    alert("Sample already received in lab unable to cancel the test. " + tname.innerText + " Contact administrator.");
                    status = false;
                    return false;
                }
            });
        ...
        <asp:Panel ID="panSearch" CssClass="searchPanel" runat="server" meta:resourcekey="panSearchResource1">
            ...
            <table class="w-100p dataheader3">
                <tr>
                    ...
                    <td class="a-left" style="width: 100px;">
                        <asp:Label ID="lblvisit" runat="server" Text="Visit Number" meta:resourcekey="lblvisitResource1"></asp:Label>
                    </td>
                    <td class="a-left" style="width: 100px;">
                    ...
                    <td class="a-left">
                        <asp:Button ID="btnGo" runat="server" CssClass="btn" meta:resourcekey="btnGoResource1"
                            ...
                            Text="Search" ToolTip="Click here to Search the Tests For Cancellation" />
                    </td>
                    <td class="defaultfontcolor" style="display: none">
                ...
                <tr>
                    <td>
                        <div style="max-width: 100%; max-height: 400px; overflow: scroll">
                            ...
                            <asp:GridView ID="grdResult" runat="server" CssClass="gridView w-100p" AutoGenerateColumns="False"
                                PagerStyle-ForeColor="black" OnRowDataBound="grdResult_RowDataBound" meta:resourcekey="grdResultResource1">
                                <Columns>
                                    <asp:TemplateField>
                                        <HeaderTemplate>
                                    ...
                                    <asp:TemplateField HeaderText="InvestigationID" Visible="false" meta:resourcekey="TemplateFieldResource2">
                                        <ItemTemplate>
                                            <asp:Label ID="lblInvestigation" runat="server" Text='<%# Bind("ID") %>' Visible="False"></asp:Label>
                                            ...
                                            <asp:Label ID="lblPkgID" runat="server" Text='<%# Bind("PkgID") %>' Visible="False"></asp:Label>
                                            <asp:Label ID="lblPkgName" runat="server" Text='<%# Bind("PkgName") %>' Visible="False"></asp:Label>
                                        </ItemTemplate>
                                    </asp:TemplateField>
                ...
                <tr id="trApprover" runat="server" style="display: table-row;">
                    <td class="bold h-20 w-5p" style="color: #000;">
                        ...
                        <ajc:AutoCompleteExtender ID="AutoAuthorizer" runat="server" CompletionInterval="10"
                            ...
                            CompletionListItemCssClass="wordWheel itemsMain" EnableCaching="False" Enabled="True"
                            MinimumPrefixLength="1" FirstRowSelected="true" ServiceMethod="getUserNamesWithNameandLoginID"
                ...
                <tr>
                    <td class="a-center">
                        ...
                        <asp:Button Text="Save" ID="btnSubmit" runat="server" ToolTip="Click here to Save the Details"
                            ...
                            onmouseout="this.className='btn'" OnClick="btnSubmit_Click" OnClientClick="javascript: if(Cleartxt()){return ValidateIfSampleReceivedOrNot()}else {return false;};"
                            meta:resourcekey="btnSubmitResource1" />
                        <asp:Button ID="btnReject" runat="server" Text="Reject" ToolTip="Click here to Reject"
                            Style="cursor: pointer;" CssClass="btn" onmouseover="this.className='btn btnhov'"
                            onmouseout="this.className='btn'" OnClientClick="javascript:return ValidationForRejection();"
