...
public partial class Lab_TransferSampleCollection : BasePage
{
    public Lab_TransferSampleCollection()
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                ddlMultiRoleuser.DataSource = lstusers;
                ddlMultiRoleuser.DataTextField = "Name";
                ddlMultiRoleuser.DataValueField = "LoginID";
                ddlMultiRoleuser.DataBind();
                ddlMultiRoleuser.Items.Insert(0, strSelects.Trim());
                ddlMultiRoleuser.Items[0].Value = "0";
                ddlMultiRoleuser.Items[0].Text = "--SELECT--";
               // ddlMultiRoleuser.SelectedValue = LID.ToString();
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        ...
        if (lstpinvsampleVisits.Count == 0)
        {
            returncode = new Investigation_BL(base.ContextInfo).UpdateSampleTransfer(lstpinvsample);
    ...
    public void LoadLocationNEW()
    {
        ...
        if (lstOrganizationAddress.Count > 0)
        {
            if (CID > 0)
            {
                ...
                ddlLocationn.DataSource = LoginLoc;
                ddlLocationn.DataValueField = "AddressID";
                ddlLocationn.DataTextField = "Location";
                ddlLocationn.DataBind();
            }
            else
            {
                ddlLocationn.DataSource = lstOrganizationAddress;
                ddlLocationn.DataValueField = "AddressID";
                ddlLocationn.DataTextField = "Location";
                ddlLocationn.DataBind();
            }
    ...
    private void LoadLocation()
    {
        ...
        if (lstLocation.Count > 0)
        {
            ...
            //added by sudhakar//
                if (Request.QueryString["Flag"] == "Y")
                {
                    if (GetConfigValue("IsDefaultMainlab", OrgID) == "Y")
                    {//DefaultSelectedLocation
                        //MainlabinLilavathi
                        long locid = 0;
                        long.TryParse(GetConfigValue("DefaultSelectedLocation", OrgID), out locid);
                        if (locid > 0)
                        {
                            if (lstLocation.Any(cus => cus.AddressID == locid))
                            {
                                ddltransferloc.SelectedValue = locid.ToString();
                            }
                        }
                    } else
                {
    ...
    private void LoadGrid(EventArgs e, int currentPageNo, int PageSize)
    {
        ...
        {
            if (txtVisitID.Text != "" && txtVisitID.Text != "Visit Number" && txtVisitID.Text != "Lab Number")//added by jegan
            {
                visittype = Convert.ToInt32(ddVisitType.SelectedValue);
            }
            else if (!string.IsNullOrEmpty(ddVisitType.SelectedValue))
            {
                visittype = Convert.ToInt32(ddVisitType.SelectedValue);
            }

        }
        if (ddlLocationn.SelectedValue != "-1")
        {
            Locationn = Convert.ToInt32(ddlLocationn.SelectedValue);
        }
        ...
        returncode = invbl1.GetInvSamplesTransferStatus(OrgID, txtFrom.Text, txtTo.Text, intSampleStatus, CollectedLocationID, "", visitid, patientname,
            visittype, priority, sourcename, InvestigationName, InvestigationID, InvestigationType, refPhyName, refPhyID, refPhyOrg,
           SmID, Convert.ToInt32(ddlType.SelectedItem.Value), out lstInvestigationSamples, PageSize, currentPageNo, out totalRows, smpleID, ProcessedLocID, outsourceID, BarcodeNo, ContainerID, PlocationID, Convert.ToInt32(ddlLocationn.SelectedValue),Convert.ToInt64(ddlMultiRoleuser.SelectedValue));
        //
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("../Lab/TransferSampleCollection.aspx?Flag=Y", true);
        }
