...
public partial class Lab_InvestigationResultDeflagging : BasePage
{
    public Lab_InvestigationResultDeflagging()
        : base("Lab_InvestigationResultDeflagging_aspx")
    {
    }
    ...
    public void grdResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            /* BEGIN | NA | Sabari | 20181202 | Created | InvestigationResultDeflagging */
            ddlRole.Attributes.Add("onchange", "javascript:DropCheck('" + ddlRole.ClientID + "','" + chkSelect.ClientID + "');");
            /* END | NA | Sabari | 20181202 | Created | InvestigationResultDeflagging */
            ...
            long returncode = -1;
            string domains = "statusorg,InvestigationResultDeflag_Role,InvestigationResultDeflag_Status";
            ...
            string InvestigationResultDeflag = GetConfigValues("InvestigationResultDeflag", OrgID);
            ...
            {
                var childItemsInvestigationResultDeflagstatus = from child in lstmetadataOutput
                                                                //where child.Domain == "statusorg"
                                                                where child.Domain == "InvestigationResultDeflag_Status"
                                                                select child;
                var childItemsddlRole = from child in lstmetadataOutput
                                        where child.Domain == "InvestigationResultDeflag_Role"
                                        select child;
                if (childItemsInvestigationResultDeflagstatus.Count() > 0)
                {
                    if (InvestigationResultDeflag == "Y")
                    {
                        ddlstatus.DataSource = childItemsInvestigationResultDeflagstatus;
                    ...
                    //}
                    /*SAbari Commented End*/

                }
                /* BEGIN | NA | Sabari | 20181202 | Created | InvestigationResultDeflagging */
                /*Bind Role DropDown*/
                if (childItemsddlRole.Count() > 0)
                {
                    if (InvestigationResultDeflag == "Y")
                    {
                        ddlRole.DataSource = childItemsddlRole;
                        ddlRole.DataTextField = "DisplayText";
                        ddlRole.DataValueField = "Code";
                        ddlRole.DataBind();

                    }
                    if (InvestigationResultDeflag == "Y" && OI.Status == "ResultDeflag")
                    {
                        ddlRole.SelectedItem.Text = "Pathologist";
                    }
                    else if (InvestigationResultDeflag == "Y" && OI.Status == "ResultDeflags")
                    {
                        ddlRole.SelectedItem.Text = "SrLabTech";
                    }
                }
                /* END | NA | Sabari | 20181202 | Created | InvestigationResultDeflagging */
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in grdResult.Rows)
        {
            ...
            if (chkBox.Checked)
            {
                ...
                lstitem.RefPhyName = ddlReason.SelectedItem.Text;
                lstitem.RoleID = Convert.ToInt64(ddlRole.SelectedItem.Value);
                lstitem.RoleName = ddlRole.SelectedItem.Text;
                /* END | NA | Sabari | 20181202 | Created | InvestigationResultDeflagging */
        ...
        {
            //Status changed Based on Role [Pathologist=ResultDeflag,StLabTech=ResultDeflags]
            /* BEGIN | NA | Sabari | 03032019 | Created | InvestigationResultDeflagging */
            foreach (OrderedInvestigations item in lstOrderinvestication)
            {
                if (item.Status == "ResultDeflag" && item.RoleName == "SrLabTech")
                {
                    item.Status = "ResultDeflags";
                }
            ...
            /*  Commented  Start: If we use Foreach Multiple Task Created against ResultDeflag for Same visit */

            //foreach (var lst in lstOrderinvestication)//deep
            //{
                
            //    if (lst.Status == "ResultDeflag" && lst.RoleName == "Pathologist")
            //    {
            ...
            //        gUID = hdnUID.Value;
            //        returncode = Utilities.GetHashTable(Convert.ToInt64(TaskHelper.TaskAction.ResultDeflag),
            ...
            //        task.TaskActionID = 98;
            //        task.DispTextFiller = dText;
            //        task.URLFiller = urlVal;
            //        //task.RoleID = RoleID;
            //        task.RoleID = lst.RoleID;
            ...
            //        //break;
            //    }
            //    if (lst.Status == "ResultDeflags" && lst.RoleName == "SrLabTech")
            //    {
            //        Tasks_BL oTasksBL = new Tasks_BL(base.ContextInfo);
            //        long createTaskID = -1;
            //        Tasks task = new Tasks();
            ...
            //        string ClientID = string.Empty;
            //        gUID = hdnUID.Value;
            //        returncode = Utilities.GetHashTable(Convert.ToInt64(TaskHelper.TaskAction.ResultDeflagSrLabTech),
            //                                  vid, 0, PatientID, lstPatientVisitDetails[0].TitleName + " " +
            //                                  lstPatientVisitDetails[0].PatientName, "", 0, "", 0, "", 0, "INV"
            //                                  , out dText, out urlVal, "0", lstPatientVisitDetails[0].PatientNumber, 0,
            //                                  gUID, lstPatientVisitDetails[0].ExternalVisitID, lstPatientVisitDetails[0].VisitNumber, "");
            //        //   task.TaskActionID = Convert.ToInt32(TaskHelper.TaskAction.Revalidate);
            //        task.TaskActionID = 99;
            //        task.DispTextFiller = dText;
            //        task.URLFiller = urlVal;
            //        //task.RoleID = RoleID;
            //        task.RoleID = lst.RoleID;
            ...
            /*  Commented  END: If we use Foreach Multiple Task Created against ResultDeflag for Same visit */

            /* NEW CODE PART FOR Single visit Same time Multiple Test Deflag Means Multiple Task will Be Generated */
            if (lstOrderinvestication.Count > 0)
            {
                /*Below both linq querry return same output,but not directely return list so we can't use this here */
                //var istOrderedInvestigationsPathologistResultDeflAG1 = from p in lstOrderinvestication
                //                                                      where p.Status == "ResultDeflags" && p.RoleName == "SrLabTech"
                //                                                      select p;


                //IEnumerable<OrderedInvestigations> selectedEmployees = from p in lstOrderinvestication
                //                                                       where p.Status == "ResultDeflag" && p.RoleName == "Pathologist"
                //                                                       select p;
                /*Below both linq querry return same output */


                /* Linq querry to Filter Result Deflag Test's based on status to avoid Multiple TaskTable insert's */

                List<OrderedInvestigations> lstOIResultDeflag = lstOrderinvestication.FindAll(p => p.Status.Contains("ResultDeflag") && p.RoleName == "Pathologist");

                List<OrderedInvestigations> lstOIResultDeflags = lstOrderinvestication.FindAll(p => p.Status.Contains("ResultDeflags") && p.RoleName == "SrLabTech");

                var lstOIResultDeflagPathologist = lstOrderinvestication
                                                    .Where(p => p.Status == "ResultDeflag" && p.RoleName == "Pathologist")
                                                    .ToList();


                var lstOIResultDeflagsSrLabTech = lstOrderinvestication
                                                     .Where(p => p.Status == "ResultDeflags" && p.RoleName == "SrLabTech")
                                                     .ToList();
                
                if (lstOIResultDeflag.Count > 0 || lstOIResultDeflags.Count > 0)
                {
                    long PatientID = -1;
                    long returncode = -1;
                    vid = lstOrderinvestication[0].VisitID;
                    ...
                    /*Pathologist Result Deflag Task Creation*/

                    if (lstOIResultDeflag.Count > 0 && lstOIResultDeflagPathologist.Count > 0)
                    {
                        Tasks_BL oTasksBL = new Tasks_BL(base.ContextInfo);
                        long createTaskID = -1;
                        Tasks task = new Tasks();
                        ...
                        task.URLFiller = urlVal;
                        //task.RoleID = RoleID;
                        task.RoleID = lstOIResultDeflag[0].RoleID;
                        task.OrgID = Convert.ToInt32(OrgID.ToString());
                        task.PatientVisitID = vid;
                        task.PatientID = PatientID;
                        task.TaskStatusID = (int)TaskHelper.TaskStatus.Pending;
                    ...
                    /*SrLabTech Result Deflag Task Creation*/

                    if (lstOIResultDeflags.Count > 0 && lstOIResultDeflagsSrLabTech.Count > 0)
                    {
                        Tasks_BL oTasksBL = new Tasks_BL(base.ContextInfo);
                        long createTaskID = -1;
                        Tasks task = new Tasks();
                        ...
                        task.URLFiller = urlVal;
                        //task.RoleID = RoleID;
                        task.RoleID = lstOIResultDeflags[0].RoleID;
                        task.OrgID = Convert.ToInt32(OrgID.ToString());
                        task.PatientVisitID = vid;
                        task.PatientID = PatientID;
                        task.TaskStatusID = (int)TaskHelper.TaskStatus.Pending;
