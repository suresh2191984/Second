...
var superTable = function(tableId, options) {
    ...
    if(h<this.sParent.offsetHeight)
        this.sParentHeight = h + 7;
    else
        this.sParentHeight = this.sParent.offsetHeight;
    var w = $('#' + tableId).width();
    if (w < this.sParent.offsetWidth)
        this.sParentWidth = w + 10;
    else
    ...
    if (this.fixedCols > 0) {
        ...
        //this.sFDataInner.appendChild(this.sHeaderTable);
    }
    ...
    if (this.fixedCols > 0) {
        ...
        //this.sFData.appendChild(this.sDataTable);
    ...
    $(".sSky-Main tbody tr").each(function(i, n) {
        ...
        $(this).find("td > input").focus(function(e) {
            $(this).parent().parent()[0].style.backgroundColor = "#786876";
        }).blur(function(e) {
            $(this).parent().parent()[0].style.backgroundColor = "White";
        });
        $(this).find("td > select").focus(function(e) {
            $(this).parent().parent()[0].style.backgroundColor = "#786876";
        }).blur(function(e) {
            $(this).parent().parent()[0].style.backgroundColor = "White";
        });
        $(this).find("td > input").keyup(function(e) {
            var $thisRow = $(this).parent().parent();
            visitStatus = "Completed";
            $thisRow.each(function() {
                $(this).find("td > input:text:enabled").each(function() {
                    if ($.trim($(this).val()) === "") {
                        visitStatus = "Pending";
                ...
                if (visitStatus == "Completed") {
                    $(this).find("td > select:not(:last)").each(function() {
                        if ($.trim($(this).val()) === "0") {
                            visitStatus = "Pending";
                            return false; // break out of the each-loop
                        }
                ...
                if (visitStatus == "Pending") return false;
            });
            var $stsDrp = $thisRow.children('td:last').find('select');
            if ($stsDrp.length > 0) {
                $stsDrp.val(visitStatus);
            }
        });

        $(this).find("td > select:not(:last)").change(function(e) {
            var $thisRow = $(this).parent().parent();
            visitStatus = "Completed";
            $thisRow.each(function() {
                $(this).find("td > input:text:enabled").each(function() {
...
function CheckIfEmptyBatchSheet(id) {
    //debugger;
    if (id.value === "Completed") {
        var CheckStatus = "Pending";
        var $thisRow = $(id).parent().parent();
        $thisRow.each(function() {
            $(this).find("td > input:text:enabled").each(function() {
                if ($.trim($(this).val()) === "") {
                    CheckStatus = "Pending";
...
//            if (CheckStatus == "Completed") {
        ...
        var $stsDrp = $thisRow.children('td:last').find('select');
        if ($stsDrp.length > 0 && CheckStatus == "Pending") {
            alert('No Value Entered, Please Check Investigation status');
            $stsDrp.val(CheckStatus);
            return false;
        }
		return true;
    }
