...
var IsFromDate = "N"; ////It Notify whether it is FromDate(Y) value or ToDate(N) Value, If it is FromDate Value, then Default time is 12Am, Or else Current time
...
var SundayColor = "#9799A6"; //var SundayColor="#C0F64F";//Background color of Sunday.
var SaturdayColor = "#9799A6"; //Background color of Saturday.
var WeekDayColor = "#9799A6"; //Background color of weekdays.
var FontColor = "blue"; //color of font in Calendar day cell.
var TodayColor = "#EF9103"; //var TodayColor="#FFFF33";//Background color of today.
...
var DisableAfterToday = "N"; //Make date after today unclickable.
...
function NewCssCall(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pHideSeconds, pIsFromDate, PastDateDisable) {

    if (PastDateDisable == "true")
    {
        DisableBeforeToday = true;
    }
    if (PastDateDisable == "Y") {
        DisableAfterToday = "Y";
    }
    // get current date and time
    if (pIsFromDate != null) {
        IsFromDate = pIsFromDate;
    }
    else {
        if (pIsFromDate != "Y")
        { IsFromDate = "N" }
    ...
    RenderCssCal(true, pCtrl);
}
function NewCssCal(pCtrl,pFormat,pScroller,pShowTime,pTimeMode,pHideSeconds,pIsFromDate) {
    // get current date and time
    if (pIsFromDate != null) {
        IsFromDate = pIsFromDate;
    }
...
function RenderCssCal(bNewCal, pCtrl) {
	...
	vCalHeader="<table  "+CalBgColor+" background='"+ThemeBg+"' border=1 cellpadding=1 cellspacing=1 width='200px' style='background-color: #E6E6E6;' valign='top'>\n";
  ...
  //******************End Month and Year selector in arrow******************************
...
else if (DisableAfterToday == "Y" && ((j > dtToday.getDate()) && (Cal.Month === dtToday.getMonth()) && (Cal.Year === dtToday.getFullYear()) || (Cal.Month > dtToday.getMonth()) && (Cal.Year === dtToday.getFullYear()) || (Cal.Year > dtToday.getFullYear()))) {
	...
	if (Cal.ShowTime) 
	{
		...
		vCalTime += "<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px\" value=" + showmins + " onChange=\"javascript:Cal.SetMinute(this.value)\">";
	
		if (Cal.ShowSeconds) {
			vCalTime+="</td><td align='center'>:</td><td align='center' width='22px'>";
			vCalTime += "<input type='text' name='second' maxlength=2 size=1 style=\"WIDTH: 22px\" value=" + showsec + " onChange=\"javascript:Cal.SetSecond(parseInt(this.value,10))\">";
		}
	...
	if (winCal == undefined) {
     ...
     // add javascript function to the span cal
	   ...
	   var cssStr = ".calTD {font-family: verdana; font-size: 12px; text-align: center; border:0;color:#000000 }\n";
       ...
       with (span.style) {position = "absolute"; left = (xpos-200)+'px'; top = (ypos+18)+'px'; width = CalWidth+'px'; border = "solid 2pt " + SpanBorderColor; padding = "0pt"; cursor = "move"; backgroundColor = SpanBgColor; zIndex = 100;}
	  ...
	  if(bNewCal==true){
	     winCal.style.left = (xpos-200)+'px';
	     winCal.style.top = (ypos+18)+'px';
	   }
	}
...
function getShowHour() {
	...
	if (document.getElementById('txtSampleDate') != undefined && document.getElementById('txtSampleDate') != '') {
