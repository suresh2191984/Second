...
function IAmSelected(source, eventArgs) {
    ...
    if ((AvilableQty > ReorderQty) || (AvilableQty >= ReorderQty)) {
...
function KeyPress1(e) {
    ...
    if (ddlaction.options[ddlaction.selectedIndex].text == '--Select--') {
        alert('Select The Issued And  Received By');
        return false;
    }
}
        ...
        for (i = 0; i < BatchNoList.length; i++) {
            if (BatchNoList[i] != "") {
                ...
                if (val[0].toUpperCase() == (document.getElementById('txtBatchNo').value.trim()).toUpperCase()) {
                    ...
                    document.getElementById('hdnUnitPrice').value = val[16];
                    ToInternalFormat($('#hdnUnitPrice'));
                    document.getElementById('hdnParentProductID').value = val[17];
...
function BindProductList() {
    if (document.getElementById('add').value == 'Update') {
        ...
        var pName = document.getElementById('hdnProductName').value;
        var pProductId = document.getElementById('hdnProductId').value;
        var pQTY = ToInternalFormat($('#txtBatchQuantity'));  //document.getElementById('txtBatchQuantity').value;
        var pBatchNo = document.getElementById('txtBatchNo').value;
        var pQuantity = ToInternalFormat($('#txtQuantity')); //document.getElementById('txtQuantity').value;
        var pUnit = ToInternalFormat($('#txtUnit')); //document.getElementById('txtUnit').value;
        var pSellingPrice = ToInternalFormat($('#hdnSellingPrice')); //document.getElementById('hdnSellingPrice').value;
        var pTax = ToInternalFormat($('#hdnTax')); //document.getElementById('hdnTax').value;
        ...
        var PUnitPrice = ToInternalFormat($('#hdnUnitPrice'));  //document.getElementById('hdnUnitPrice').value;
        ...
        document.getElementById('hdnProductList').value += pId + "~" + pName + "~" +
                        ...
                        pProductId + "~" + pSellingPrice + "~" + pTax + "~" + pExp + "~" + pIntendID + "~" + pCategoryID + "~" + pStockInHandID + "~" + PUnitPrice +"~"+pParentProductID+ "^";
        Tblist();
        document.getElementById('txtQuantity').value = '';
        document.getElementById('txtUnit').value = '';
    ...
    if (x.length > 1) {
        ...
        for (i = 0; i < x.length; i++) {
            if (x[i] != "") {
                ...
                cell1.innerHTML = y[1];
                cell2.innerHTML = y[2];
                $('#hdnTargetMsg').val(y[3]);
                cell3.innerHTML = ToTargetFormat($('#hdnTargetMsg'));

                $('#hdnTargetMsg').val(y[4]);
                cell4.innerHTML = ToTargetFormat($('#hdnTargetMsg'));

                $('#hdnTargetMsg').val(y[7]);
                cell5.innerHTML = ToTargetFormat($('#hdnTargetMsg'));

                $('#hdnTargetMsg').val(parseFloat(y[3] * y[7]).toFixed(2));
                cell6.innerHTML = ToTargetFormat($('#hdnTargetMsg'));
                
                tGrandTotal = parseFloat(parseFloat(tGrandTotal) + parseFloat(y[3] * y[7])).toFixed(2);
                cell7.innerHTML = "<input name='" + y[0] + "~" + y[1] + "~" + y[2] + "~" + y[3] + "~" + y[4] + "~" + y[5] + "~" + y[6] + "~" + y[7] + "~" + y[8] + "~" + y[9] + "~" + y[10] + "~" + y[11] + "~" + y[12] + "~" + y[13] + "~" + y[14] + "' onclick='btnEdit_OnClick(name);' value = 'Edit' type='button' style='background-color:Transparent;color:Red;border-style:none;text-decoration:underline;cursor:pointer'  /> &nbsp;&nbsp;" +
...
function btnEdit_OnClick(sEditedData) {
    ...
    ToInternalFormat($('#hdnUnitPrice'));
    document.getElementById('hdnParentProductID').value = y[14];
        ...
        for (i = 0; i < lis.length; i++) {
            if (lis[i] != "") {
               ...
               // var pdate = tblData[6];
                ...
                tblString[2] += "<tr><td>" + tblData[1] + "</td><td>" + tblData[3] + "</td><td align='left'>" + d + "</td><td>" + ToTargetFormat($('#hdnTargetMsg')) + "</td><td align='right'>" + tblData[6] + "(" + ToTargetFormat($('#hdnTargetMsg1')) + ")" + "</td></tr>";
                sum += parseFloat(tblData[4]);
                unit = "(" + tblData[6] + ")";
            }
        }
        $('#hdnTargetMsg').val(sum.toFixed(2) + unit);
        tblString[2] += "<tr><td colspan='4' align='right'>Total</td><td align='right'>" + ToTargetFormat($('#hdnTargetMsg')) + "</td></tr>";
