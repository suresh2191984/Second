...
function LoadData() {
    ...
    fnGetFoodOrderedList(orgID, pno, totalCount);
}

function fnGetFoodOrderedList(OrgID, pno, TotalRows) {
    $.ajax({
        type: "POST",
        url: "../PagingService.asmx/ProcessCompletedFoodDetails",
        data: "{ 'OrgID': '" + OrgID + "','pno': '" + pno + "','totalRows': '" + TotalRows + "'}",
...
function fnBindFoodRows(Items) {
    $('#gvCompletedFoodDetails').empty();
    $('#gvCompletedFoodDetails').append('<tr class="dataheader1"><th scope="col">Select</th><th scope="col">Ward Name</th><th scope="col">Patient Name</th><th scope="col">Food Name</th><th scope="col">Food Ordered Date</th></tr>');
    $.each(Items, function(index, Item) {
        $('#gvCompletedFoodDetails').append('<tr><td><input type="checkbox" onclick="GetDeliveringDetails(this)" id="chk" RI="' + Item.Rowid + '"  FI="' + Item.FoodOrderID + '" /></td><td>' + Item.WardName + '</td><td>' + Item.Name + '</td><td>' + Item.FoodName + '</td><td>' + JSONDateWithTime(Item.FoodOrderDate) + '</td></tr>');
    });
    MakeChkBoxSelect();
