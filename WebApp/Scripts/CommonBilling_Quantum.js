function clearBillPartValues() {
    ...
    document.getElementById('billPart_hdnStstFee').value = '0.00';
    document.getElementById('billPart_hdnAutoCalcGenBillItemID').value = '0';
    document.getElementById('billPart_hdnAutoCalcGenBillItemAmt').value = '0';
    document.getElementById('billPart_hdnStatAutoFeeID').value = '0';
    document.getElementById('billPart_hdnActualGenBillItemAmt').value = '0';
    
    document.getElementById('hdnCheckFlag1').value = 'Y';
    document.getElementById('billPart_hdnRateForStat').value = '';
    document.getElementById('billPart_hdnBilledDate').value='';
...
function ClientSelected(source, eventArgs) {
    //debugger;
    var pClientName = eventArgs.get_text();
    if (document.getElementById('hdnClientName').value != pClientName) {
        document.getElementById('billPart_hdnRtID').value = '';
    }
    var ClientCorpID;
    var ClientCorpName;
    var ClientCorpCode;
    ...
    if (slist.length > 0) {
        for (j = 0; j < slist.length - 1; j++) {
            ...
            if (j == 0) {
                ...
                PRateType = flist[20];
                ExAutoAuthorization = flist[21];
                RefHospitalName = flist[28];
            }
            if (temp > flist[8]) {
                ClientStatus = flist[13].trim();
                if (ClientStatus == 'S' || ClientStatus == 'T') {
   ...
   
    {
        ...
        $("[id*=chkDespatchMode] input:checkbox").prop('checked', true);
        ...
        $('#FaxNumber').val('');
        $("[id*=chkDespatchMode] input:checkbox").prop('checked', false);
    }

    document.getElementById('hdnfaxnumber').value = FaxNumber;

    document.getElementById('hdnSelectedClientMappingID').value = ClientCorpMappingID;
    if (ClientCorpCode == 'GENERAL') {
        document.getElementById('hdnIsCashClient').value = 'Y';
    ...
    if (ClientType.trim() == 'HOS') {
        ...
        if (RefHospitalName == '') {
            ...
            document.getElementById('txtReferringHospital').value = RefHospitalName;
        }
...
function countQuickAge(id) {
    ...
    if (document.getElementById(id).value != '') {
        ...
        if (main == "valid") {
            ...
            var age = getAge(mm + '/' + dd + '/' + yy, gmonth + 1 + '/' + gdate + '/' + gyear);
            if (age == 0)
                age = age;
            agetemp = age;
            if (mm <= (gmonth + 1))
            ...
            document.getElementById('hdnCalculateDays').value = totdays;
            months = age * 12;
            var t = parseInt(mm);
            months += 12 - mm;
            months += gmonth + 1;
            ...
            if (agetemp <= 0) {
                ...
                if (agetemp == 1) {
                    ...
                    document.getElementById('txtDOBNos').value = getAge(mm + '/' + dd + '/' + yy, gmonth + 1 + '/' + gdate + '/' + gyear);
                    document.getElementById('ddlDOBDWMY').value = 'Year(s)';
                }

                else {
                    var AgeVal = getAge(mm + '/' + dd + '/' + yy, gmonth + 1 + '/' + gdate + '/' + gyear);
                    document.getElementById('txtDOBNos').value = AgeVal;
                    document.getElementById('ddlDOBDWMY').value = 'Year(s)';
                }
            }
            ...
            if (document.getElementById('hdnDoFrmVisit').value == "") {
                ...
                document.getElementById('ddlDOBDWMY').focus();
            }
        }
        else {            
            alert(main + ' Date');
            document.getElementById('txtDOBNos').value = '';
            document.getElementById('tDOB').value = dateformat;
            document.getElementById('tDOB').value = dateformat;
...
function AddBillingItemsDetails() {
   ...
   // debugger;
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../OPIPBilling.asmx/GetBillingItemsDetails_Quantum",
        data: JSON.stringify({ OrgID: document.getElementById('billPart_hdnOrgIDC').value, FeeID: document.getElementById('billPart_hdnID').value, FeeType: document.getElementById('billPart_hdnFeeTypeSelected').value, Description: document.getElementById('billPart_txtTestName').value, ClientID: $('[id$="hdnSelectedClientClientID"]').val(), VisitID: 0, Remarks: '', IsCollected: document.getElementById('billPart_hdnIsCollected').value, CollectedDatetime: document.getElementById('billPart_hdnCollectedDateTime').value, locationName: document.getElementById('billPart_hdnLocName').value, OrderedItem: document.getElementById('hdnordereditems').value, ExtVisitNumber: document.getElementById('txtExternalVisitID').value, BilledDate: Billdate, BillNo: BillNo }),
        dataType: "json",
        success: function(data) {
            if (data.d.length > 0) {
                for (var i = 0; i < data.d.length; i++) {
                    ...
                    if (arrGotValue.length > 0) {
                        ...
                        TxtInvoiceMsg = arrGotValue[20];
                        AutoCalcGenBillItems = arrGotValue[22];
                        if (Billdate == '') {
                            Billdate = arrGotValue[21];
                            document.getElementById('billPart_hdnBilledDate').value = Billdate;
                        }
                        datetime = arrGotValue[21];
                        if (TxtInvoiceMsg != '' && TxtInvoiceMsg != 'undefined') {
                            var AlertInvoiceMsg = confirm(TxtInvoiceMsg);
                            if (AlertInvoiceMsg) {
                        ...
                        if (AutoCalcGenBillItems == 'Y') {
                            if (document.getElementById('billPart_hdnAutoCalcGenBillItemID').value == '0') {
                                document.getElementById('billPart_hdnAutoCalcGenBillItemID').value = FeeID;
...
function getAge(fromdate, todate) {
    ...
    age.join('');
    if (document.getElementById('hdnDecimalAgeConfig').value == 'Y') {
        return ydiff + '.' + mdiff;
    }
    else {
...
function BillingItemSelected(source, eventArgs) {
    ...
    if (list.length > 0) {
        if (list[i] != "") {
            ...
            IsOutSource = list[4];
            outRInSourceLocation = list[5];
            IsOptionalTest = list[8];
            document.getElementById('billPart_hdnIsOptionalTest').value = IsOptionalTest;
            document.getElementById('billPart_hdnID').value = ID;
            document.getElementById('billPart_hdnName').value = name;
            document.getElementById('billPart_hdnFeeTypeSelected').value = feeType;
            document.getElementById('billPart_hdnInvCode').value = Code;
...
//    }
    ...
    // }
    var ClientID = document.getElementById('hdnSelectedClientClientID').value;
    if (document.getElementById('txtClient').value == '' || ClientID == 0) {
...
function CmdAddBillItemsType_onclick(FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID) {
    if (document.getElementById('txtClient').value.trim() == '' && document.getElementById('hdnDefaultOrgBillingItems').value != '' && defaultbillflag == 0) {
        ...
        FeeViewStateValue = "FeeID^" + defalutdata[0] + "~FeeType^" + defalutdata[1] + "~Descrip^" + defalutdata[2] + "~Amount^"
                ...
                + "~IsTaxable^" + defalutdata[10] + "~IsRepeatable^" + defalutdata[11] + "~IsSTAT^" + defalutdata[12] + "~IsSMS^" + defalutdata[13] + "~IsOutSource^" + defalutdata[14] + "~IsNABL^" + defalutdata[15] + "~BillingItemRateID^" + document.getElementById('hdnRateID').value + "~Code^" + defalutdata[16] + "~HasHistory^" + "N" + "~outRInSourceLocation^" + "N" + "|";
        document.getElementById('billPart_hdfBillType1').value += FeeViewStateValue;

    }
    // Modified By Sathish.E--------Begin---------//
    var test = document.getElementById('hdnNeedAutoStatFee').value;
    ...
    //    if (document.getElementById('billPart_hdnRateForStat').value != '') {

    //        FeeViewStateValue = "FeeID^" + RatesforStatTest[0] + "~FeeType^" + RatesforStatTest[2] + "~Descrip^" + RatesforStatTest[1] + "~Amount^"
    //                        + RatesforStatTest[3] + "~Quantity^" + RatesforStatTest[4] + "~Remarks^" + RatesforStatTest[5] + "~ReportDate^" + RatesforStatTest[5] + "~IsReimbursable^" + RatesforStatTest[7] + "~ActualAmount^" + RatesforStatTest[8]
    //                        + "~IsDiscountable^" + RatesforStatTest[9] + "~IsTaxable^" + RatesforStatTest[10] + "~IsRepeatable^" + RatesforStatTest[11] + "~IsSTAT^" + RatesforStatTest[12] + "~IsSMS^" + RatesforStatTest[13] + "~IsOutSource^" + RatesforStatTest[14]
    //                        + "~IsNABL^" + RatesforStatTest[15] + "~BillingItemRateID^" + RatesforStatTest[16] + "~Code^" + RatesforStatTest[17] + "~HasHistory^" + "N" + "~outRInSourceLocation^" + "N" + "|";
    //        document.getElementById('billPart_hdfBillType1').value += FeeViewStateValue
    //    }

    // Modifed by Sathish.E-------End------------//
    ...
    if (iPaymentAlreadyPresent == 0) {
        FeeViewStateValue = "FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^"
                ...
                + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "|" + FeeViewStateValue;

        document.getElementById('billPart_hdfBillType1').value = FeeViewStateValue;
        CreateBillItemsTable(0);
	document.getElementById('billPart_hdnID').value = FeeID;
        if (document.getElementById('billPart_hdnIsOptionalTest').value == 'Y') {
            var Pname = FeeID + "," + FeeType + "," + Descrip;
            GetGroupName(Pname);
	    }
...
function chkChange(value, objid) {
    ...
    document.getElementById('billPart_hdfBillType1').value = '';
    document.getElementById('billPart_hdnIsStatValues').value = '';
    var arrayMainData = new Array();
    var arraySubData = new Array();
    var arrayChildData = new Array();
    var FeeID, FeeType, Descrip, Quantity, Amount, Remarks, ReportDate, IsReimbursable, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID;
    var GrossAmt = 0;
    ...
    for (iMain = 0; iMain < arrayMainData.length; iMain++) {
        ...
        if (arrayChildData.length > 0) {
            ...
            if (arrayChildData[0] == "IsSTAT") {
                IsSTAT = istat;
                document.getElementById('billPart_hdnIsStatValues').value = IsSTAT;
            }
    ...
    // --------Changes for Adding STAT Processing Fee------------//
    //---------Modified By Sathish.E----------------------//
    if (document.getElementById('billPart_hdnIsStatValues').value == 'Y') {
        GetRateForSTAT(); //New Json WebService for Getting Rate for STAT for Client
    }
    else {
        document.getElementById('billPart_hdnRateForStat').value = '';
    }
...
function CreateBillItemsTable(id) {
    ...
    startPaymentTag = "<TABLE nowrap='nowrap' ID='tabDrg1' Cellpadding='0' Cellspacing='0' width='100%' class='dataheaderInvCtrl bg-row b-grey' style='font-size: 11px;' ><TBODY><tr class='dataheader1'><th scope='col'  style='width:5%;display:none;'> FeeID </th><th scope='col'  style='width:5%;display:none;'> FeeType </th> "
    ...
    + "<th scope='col' style='width:20%;padding-left:2px;display:none;'>Remarks </th> <th scope='col' style='align:right;width:15%;display:table-cell;'> Report Date </th> <th scope='col' style='display:none;'> IsReimbursable </th><th scope='col' style='display:none;'> ActualAmount </th><th scope='col' style='display:none;'> IsDiscountable </th><th scope='col' style='display:none;'> BaseRateID </th>"
    ...
    var EmergencyStat = 'N';
    var defalutdata = document.getElementById('hdnDefaultOrgBillingItems').value.split('^');
    var RatesforStatTest = document.getElementById('billPart_hdnRateForStat').value.split('^');
    if (id == 0) {
        if (document.getElementById('txtClient').value == '' && document.getElementById('hdnDefaultOrgBillingItems').value != '' && defaultbillflag == 0) {
            ...
            FeeViewStateValue = "FeeID^" + defalutdata[0] + "~FeeType^" + defalutdata[1] + "~Descrip^" + defalutdata[2] + "~Amount^"
                        ...
                        + "~IsNABL^" + defalutdata[15] + "~BillingItemRateID^" + defalutdata[16] + "~Code^" + defalutdata[17] + "~HasHistory^" + "N" + "~outRInSourceLocation^" + "" + "|";
            document.getElementById('billPart_hdfBillType1').value += FeeViewStateValue;

        }
    }
    //---------Modified By Sathish.E----------------------//
    //Getting Values for the STAT Tests//
    var StatAdded = document.getElementById('hdnCheckFlag1').value;
    if (document.getElementById('billPart_hdnRateForStat').value != '' && document.getElementById('billPart_hdnRateForStat').value != 'undefined' && StatAdded == 'Y') {

        FeeViewStateValue = "FeeID^" + RatesforStatTest[0] + "~FeeType^" + RatesforStatTest[2] + "~Descrip^" + RatesforStatTest[1] + "~Amount^"
                        + RatesforStatTest[3] + "~Quantity^" + 1 + "~Remarks^" + RatesforStatTest[5] + "~ReportDate^" + RatesforStatTest[5] + "~IsReimbursable^" + RatesforStatTest[7] + "~ActualAmount^" + RatesforStatTest[8]
                        + "~IsDiscountable^" + RatesforStatTest[9] + "~IsTaxable^" + RatesforStatTest[10] + "~IsRepeatable^" + RatesforStatTest[11] + "~IsSTAT^" + RatesforStatTest[12] + "~IsSMS^" + RatesforStatTest[13] + "~IsOutSource^" + RatesforStatTest[14]
                        + "~IsNABL^" + RatesforStatTest[15] + "~BillingItemRateID^" + RatesforStatTest[16] + "~Code^" + "GNFEE" + "~HasHistory^" + "N" + "~outRInSourceLocation^" + "" + "~BaseRateID^" + BaseRateID + "|";
        document.getElementById('billPart_hdfBillType1').value += FeeViewStateValue
        document.getElementById('hdnCheckFlag1').value = 'N';
            EmergencyStat = 'Y';
    }
    ...
    if (arrayMainData.length > 0) {
        for (iMain = 0; iMain < arrayMainData.length - 1; iMain++) {
            ...
            if (document.getElementById('billPart_hdnIsBillable').value.trim() == "N") {
                if (IsSTAT == 'Y') {
                    newPaymentTables += "<TD style='display:table-cell;' align='Center'><input  style='display:none;'  id='" + str + "' value='FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^" + Amount + "~Quantity^" + Quantity + "~Remarks^" + Remarks + "~ReportDate^" + ReportDate + "~IsReimbursable^" + IsReimbursable + "~ActualAmount^" + ActualAmount + "~IsDiscountable^" + IsDiscountable + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "' onclick='chkChange(value,this.id);'  name='chkAlls' checked='true'   type='checkbox'  /></TD>";
                }
                else {
                    if (FeeType == "INV" || FeeType == 'GRP' || FeeType == 'PKG') {
                        newPaymentTables += "<TD style='display:table-cell;' align='Center'><input   style='display:none;' id='" + str + "' value='FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^" + Amount + "~Quantity^" + Quantity + "~Remarks^" + Remarks + "~ReportDate^" + ReportDate + "~IsReimbursable^" + IsReimbursable + "~ActualAmount^" + ActualAmount + "~IsDiscountable^" + IsDiscountable + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "' onclick='chkChange(value,this.id);'  name='chkAlls'   type='checkbox'  /></TD>";
                    }
                    else {
                        newPaymentTables += "<TD style='display:table-cell;' align='Center'><input   style='display:none;' id='" + str + "' value='FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^" + Amount + "~Quantity^" + Quantity + "~Remarks^" + Remarks + "~ReportDate^" + ReportDate + "~IsReimbursable^" + IsReimbursable + "~ActualAmount^" + ActualAmount + "~IsDiscountable^" + IsDiscountable + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "' onclick='chkChange(value,this.id);'  name='chkAlls' disabled='true'   type='checkbox'  /></TD>";
                    }
                }
            }
            else {
                if (IsSTAT == 'Y') {
                    newPaymentTables += "<TD style='display:table-cell;' align='Center'><input   id='" + str + "' value='FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^" + Amount + "~Quantity^" + Quantity + "~Remarks^" + Remarks + "~ReportDate^" + ReportDate + "~IsReimbursable^" + IsReimbursable + "~ActualAmount^" + ActualAmount + "~IsDiscountable^" + IsDiscountable + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "' onclick='chkChange(value,this.id);'  name='chkAlls' checked='true'   type='checkbox'  /></TD>";
                } else {
                    if (FeeType == "INV" || FeeType == 'GRP' || FeeType == 'PKG') {
                        newPaymentTables += "<TD style='display:table-cell;' align='Center'><input   id='" + str + "' value='FeeID^" + FeeID + "~FeeType^" + FeeType + "~Descrip^" + Descrip + "~Amount^" + Amount + "~Quantity^" + Quantity + "~Remarks^" + Remarks + "~ReportDate^" + ReportDate + "~IsReimbursable^" + IsReimbursable + "~ActualAmount^" + ActualAmount + "~IsDiscountable^" + IsDiscountable + "~IsTaxable^" + IsTaxable + "~IsRepeatable^" + IsRepeatable + "~IsSTAT^" + IsSTAT + "~IsSMS^" + IsSMS + "~IsOutSource^" + IsOutSource + "~IsNABL^" + IsNABL + "~BillingItemRateID^" + BillingItemRateID + "~Code^" + Code + "~HasHistory^" + HasHistory + "~outRInSourceLocation^" + outRInSourceLocation + "~BaseRateID^" + BaseRateID + "' onclick='chkChange(value,this.id);'  name='chkAlls'   type='checkbox'  /></TD>";
                    }
    ...
    if (document.getElementById('hdnEmergencyStatFee').value == 'Y') {

        CalculationForStatFee(EmergencyStat);
    }
    if (document.getElementById('billPart_hdnNeedAutoCalc').value == 'Y') {
        AutoCalcGenralBillItems();
    }
    getOrderedItems();
...
function GetGroupName(FeeId, FeeType) {
    ...
    if (FeeType != undefined && FeeId != undefined) {
        FeeIds = FeeId;
        pFeeType = FeeType;
    }
    else {
        FeeIds = FeeId.split(',')[0];
        document.getElementById('billPart_hdnID').value = FeeIds;
        pFeeType = FeeId.split(',')[1];
        pDescrip = FeeId.split(',')[2];
    }
    if (pFeeType == 'GRP' || pFeeType == 'PKG') {
        $.ajax({
            type: "POST",
...
function GetLabHistory(result) {
    ...
    for (var n = 0; n < result.length; n++) {
        ...
        if ($('input[id$="hdnPkgandgrpID"]').val() != '') {
            var lstResulType = JSON.parse($('input[id$="hdnPkgandgrpID"]').val());
            $.each(lstResulType, function(i, obj) {
                if (obj.PkgID == document.getElementById('billPart_hdnID').value) {
                    $('#tblGroupHistory tr').each(function(i, j) {
                        $parentRow = $(j);
                        $parentRow.find($('input:checkbox[id=' + obj.ID + ']').attr('checked', true));
...
function SetNetValue(obj) {
    ...
    if (Number(document.getElementById('billPart_txtGross').value) > 0) {
        ...
        if (Number(document.getElementById('billPart_hdnAmountReceived').value) <= 0) {
            ...
            NetAmount = Number(ToTargetFormat($('#billPart_txtNetAmount')));
            ...
            if (Number(document.getElementById('billPart_ddlTaxPercent').value) > 0) {
                ...
                if (document.getElementById('billPart_hdnTaxCalFromNetAmt').value == 'Y') {
                    document.getElementById('billPart_txtTax').value = parseFloat(parseFloat(Number(NetAmount)) / 100 * (Number(document.getElementById('billPart_ddlTaxPercent').value))).toFixed(2);
                }
                ToTargetFormat($('#billPart_txtTax'));
            }
            ...
            if (document.getElementById('txtClient').value == '' && obj == "ADD") {
                ...
                if (document.getElementById('hdnIsCashClient').value == "Y") {
                    document.getElementById('billPart_PaymentType_txtAmount').value = parseFloat(getOPCustomRoundoff(netvalue)).toFixed(2);
                    ToTargetFormat($('#billPart_PaymentType_txtAmount'));
                    document.getElementById('billPart_PaymentType_txtTotalAmount').innerHTML = parseFloat(getOPCustomRoundoff(netvalue)).toFixed(2);
                    ToTargetFormat($('#billPart_PaymentType_txtTotalAmount'));
...
function btnDeleteBillingItems_OnClick1(sEditedData) {
    ...
    var iPaymentAlreadyPresent = 0;
    var iPaymentCount = 0;
    var NewPaymenttempDatas = new Array();
    var StatAmt;
    var hdnValue;
    if (document.getElementById('billPart_hdfBillType1').value != '' && document.getElementById('billPart_hdfBillType1').value != 'undefined') {
        hdnValue = document.getElementById('billPart_hdfBillType1').value;
        var statIndex = hdnValue.indexOf("STATAmount");
        if (statIndex > 0) {
            var hdnLength = document.getElementById('billPart_hdfBillType1').value.split('~').length;
            StatAmt = document.getElementById('billPart_hdfBillType1').value.split('~')[hdnLength - 1];
            hdnValue = document.getElementById('billPart_hdfBillType1').value.replace(StatAmt, '|');
            hdnValue = hdnValue.substring(0, hdnValue.length - 2)
            document.getElementById('billPart_hdfBillType1').value = hdnValue + '|';
        }
        var PaymenttempDatas = document.getElementById('billPart_hdfBillType1').value;

        PaymentAAlreadyPresent = PaymenttempDatas.split('|');
    }
    for (i = 0; i < PaymentAAlreadyPresent.length - 1; i++) {

        NewPaymenttempDatas[i] = PaymentAAlreadyPresent[i] + "~Rowno^" + (i + 1);
    }
    if (NewPaymenttempDatas.length > 0) {
        for (iPaymentCount = 0; iPaymentCount < NewPaymenttempDatas.length; iPaymentCount++) {
            if (NewPaymenttempDatas[iPaymentCount].toLowerCase() == sEditedData.toLowerCase()) {
    ...
    var FeeID;
    var StatAdded = new Array();
    if (FeeGotValue.length > 0) {
        FeeID = FeeGotValue[2];

        arrayAmount = FeeID.split('^');
        StatAdded = FeeGotValue[17].split('^')[1];
        if (StatAdded == 'GNFEE') {
            document.getElementById('hdnCheckFlag1').value = 'Y';
        }
    }
    ...
    SetOtherCurrValues();
    var len = lstInvDetails.length;
    if (lstInvDetails.length > 0) {
        var PkgIDs = FeeGotValue[0].split('^')[1];
        RemovePkglstItem(PkgIDs)
    }
}
...
function ClearControlValues() {
    ...
    if (document.getElementById('billPart_hdnPkgandgrpID').value != null) {
        document.getElementById('billPart_hdnPkgandgrpID').value = "";
    }
...
function SelectedTempClient(source, eventArgs) {
    ...
    var singleValues = eventArgs.get_value().split('^');
    $('#lblClientAddress').text(singleValues[26]);
    if (singleValues != "") {
...
function ValidateClientAddress(id) {
    var len = $find('AutoCompleteExtenderClientCorp').get_minimumPrefixLength();
...
function isOtherCurrDisplay1(pType) {
    if (pType == "B") {
        document.getElementById("billPart_" + "OtherCurrencyDisplay1_tbAmountPayble").style.display = "table";
...
function ClearDOB() {
    ...
    var ageunits = document.getElementById('ddlDOBDWMY').value;


    if (document.getElementById('txtDOBNos').value >= 106) {
    ...
    if (AGE >= valAge && ageunits == 'Year(s)') {
        return false;
    }
    else if (AGE == valage1) {
        var Userval = confirm('Age is 95 Do You want to continue');
    }
}
var retVal = true;
function DuplicateInv(Id, Type) {
    ...
    $.ajax({
		...
		Item.InvestigationName + '^';
            ...
            if (setvar != "") {
                retVal = FindDuplicatesItems(setvar, FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID);
            }
            //alert(Items);
        },
        failure: function(msg) {
    ...
    if (Descrip != '' && retVal == true) {
        CallCreateItems(Id,FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID);
    }
}
function CallCreateItems(Id,FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID) {
    if (Number(Amount) <= 0) {
        if (document.getElementById('billPart_hdnZeroAmount').value == 'N' && Id == undefined) {
            alert("Item amount is zero.Kindly Map Rate for the Item...");
            return false;
        }
        else {
        ...
        if (pBill) {
            CmdAddBillItemsType_onclick(FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID);
            document.getElementById('billPart_lblInvType').innerHTML = "";
        }
    }
    else {
...
function FindDuplicatesItems(setvar, FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID) {
...
//                        return Userval;
                        var div = $(document.createElement('div')).attr({ id: 'divUpdate', title: 'Message Box' });
                        var Con = 'N';
                        var y = jQuery.noConflict();
                        var dfr = y.Deferred();
                        y(div).html("This Test is already available as a part of Ordered Package or Group.Do you want to proceed ?");
                        y(div).dialog({
                            autoOpen: true,
                            width: 400,
                            maxWidth: 410,
                            ...
                            buttons: {
                                ...
                                "Yes": function() {
                                    document.getElementById('billPart_hdnfinduplicate').value += setvar;
                                    retVal = true;
                                    CallCreateItems(0,FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID);
                                    dfr.resolve(true);
                                    y(this).dialog("close");
                                }
...
function PrintBillItemsTable() {
    ...
    startHeaderTag = startHeaderTag + "</td></tr><tr><td colspan='3'> </td></tr><tr><td colspan='3'></td></tr><tr><td colspan='3'>"
    startPaymentTag = "<TABLE nowrap='nowrap' ID='tabDrg1' Cellpadding='0' Cellspacing='0' border='1' width='100%' class='dataheaderInvCtrl bg-row b-grey' style='font-size: 12px;' ><TBODY><tr class='dataheader1'><th scope='col'  style='width:5%;display:none;'> FeeID </th><th scope='col'  style='width:5%;display:none;'> FeeType </th> <th scope='col' style='width:7%;'> S.No </th><th scope='col' align='left' style='width:65%;padding-left:2px;'> Description </th>  <th scope='col' align='right' style='display:none;width:5%;'>  Quantity </th><th scope='col' align='right' style='width:8%;'> Amount </th> <th scope='col' style='width:20%;padding-left:2px;display:none;'>Remarks </th> <th scope='col' style='align:right;width:15%;display:none;'> Report Date </th> <th scope='col' style='display:none;'> IsReimbursable </th><th scope='col' style='display:none;'> ActualAmount </th><th scope='col' style='display:none;'> IsDiscountable </th><th scope='col' style='display:none;'> IsTaxable </th><th scope='col' style='display:none;'> IsRepeatable </th><th scope='col' style='display:none;'> IsSTAT </th><th scope='col' style='display:none;'> IsSMS </th><th scope='col' style='display:none;'> IsOutSource </th><th scope='col' style='display:none;'> IsNABL </th><th scope='col' style='display:none;'> BillingItemRateID </th><th scope='col' style='display:none;'> HasHistory </th>"; // <th scope='col' align='center'>Delete</th></tr>";
    endPaymentTag = "</TBODY></TABLE>";
    newPaymentTables = startPaymentTag;
        ...
        if (arrayMainHistoryData.length > 0) {
            for (jMain = 0; jMain < arrayMainHistoryData.length - 1; jMain++) {
                ...
                if (HistoryName == "24h urine volume Collected in ml") {
                    var y = "Height :" + AttributeValueName.split('-')[0] + "CM  Weight : " + AttributeValueName.split('-')[1] + "KG";
...
function SetPreviousVisitItemsForClient() {
    ...
    if (listLen > 0) {
        tblStatr = " <div style='overflow: auto; height: 120px;width: 475px;'>"
                ...
                + "<td style='display:table-cell;width:30px;'></td><td  style='display:none;'>IsAddToday</td><td  style='display:none;'>IsOutSource</td><td  style='display:none;'>TestCode</td></tr>";
                ...
                if (predate == curdate) {
                    ...
                    if (defalutdata[0] != res[1] && defalutdata[1] != res[2]) {
                        ...
                        tblBoody += "<tr><td>" + parseInt(j + 1) + "</td><td>" + res[0] + "</td><td style='display:none;'>" + res[1] + "</td><td style='display:table-cell;'>" + res[2] +
                                "</td><td style='display:table-cell;'></td><td style='display:none;'>" + res[5] + "</td><td style='display:none;'>" + res[6] + "</td><td style='display:none;'>" + res[7] + "</td>";
        ...
        tblTotal += "<tr><td colspan='5' style='display:table-cell;' align='center'></td></tr>";
        tblEnd = "</table></div>";
...
//----------Modified By Sathish.E-------Begin--------------//
//------Json WebService for Getting Rates for STAT Fee--------//
function GetRateForSTAT() {
    // debugger;

    var arrTotalValue = new Array();
    var TotalValuesSTAT;
    var ID, name, feeType, Amount;
    var BillNo = document.getElementById('hdnqrystrBillNo').value;
    var BillDateTime = document.getElementById('billPart_hdnBilledDate').value;
    var FeeType1 = 'STAT';
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "../OPIPBilling.asmx/GetRateForSTAT",
        data: JSON.stringify({ OrgID: document.getElementById('billPart_hdnOrgIDC').value, FeeID: document.getElementById('billPart_hdnID').value, FeeType: FeeType1, ClientID: $('[id$="hdnSelectedClientClientID"]').val(), BillNo: BillNo, BillstartTime: BillDateTime }),
        dataType: "json",
        async: false,
        success: function(data) {
            if (data.d.length > 0) {
                ...
                for (var i = 0; i < data.d.length; i++) {
                    TotalValuesSTAT = data.d[0].ProcedureName;
                    arrTotalValue = data.d[0].ProcedureName.split('^');
                    //                    if (arrTotalValue.length > 0) {
                    //                        ID = arrTotalValue[0];
                    //                        name = arrTotalValue[1].trim();
                    //                        feeType = arrTotalValue[2];
                    //                        Amount = arrTotalValue[3];
                    //                    }

                    if (arrTotalValue[18].toUpperCase() == 'PERCENTAGE') {
                        document.getElementById('hdnEmergencyStatFee').value = 'Y';
                        document.getElementById('billPart_hdnStatAutoFeeID').value = arrTotalValue[0];
                    }
                    else {
                        document.getElementById('hdnEmergencyStatFee').value = 'N';
                    }
                }
            }

            document.getElementById('billPart_hdnRateForStat').value = TotalValuesSTAT;
        },
        error: function(result) {
            alert("Stat Fee Error");
        }
    });
}

//------------------ Sathish.E-------------End--------------//
function RemovePkglstItem(PkgID) {
    var len = lstInvDetails.length;
    
    for (var j = 0; j < len; j++) {
        if (lstInvDetails[j].PkgID == PkgID) {
            lstInvDetails.splice(j, 1);
            len = lstInvDetails.length;
            j--;
...
function SelecetedCode(source, eventArgs) {
    // debugger;

    var Values = "";
    Values = eventArgs.get_value();
    var pID = Values.split('^')[0];
    var pClientID = Values.split('^')[1];
    var pReferingPhysicianID = Values.split('^')[2];
    var pLabRefOrgID = Values.split('^')[3];
    var pClientName = Values.split('^')[4];
    var pPhysicianName = Values.split('^')[5];
    var RefOrgName = Values.split('^')[6];
    var ExcludeAutoauthorisation = Values.split('^')[7];
    var ID = Values.split('^')[8];
    if (ExcludeAutoauthorisation == "Y") {
        $('#chkExcludeAutoathz').attr('checked', true);
    }
    $('[id$="hdnSelectedClientClientID"]').val('');
    $('[id$="hdnReferedPhyID"]').val('');
    $('[id$="hdfReferalHospitalID"]').val('');

    $('#txtClient').val(pClientName);
    $('#txtInternalExternalPhysician').val(pPhysicianName);
    $('#txtReferringHospital').val(RefOrgName);
   
    if (document.getElementById('hdnSelectedClientID') != null) {
        document.getElementById('hdnSelectedClientID').value = pClientID;
    }
    if (document.getElementById('btnsave') != null) {
        document.getElementById('btnsave').value = "Update";
        if (document.getElementById('hdnUpdate') != null) {
            document.getElementById('hdnUpdate').value = "Y";
        }
    }
    if (document.getElementById('hdnuniqid') != null) {
        document.getElementById('hdnuniqid').value = ID;
    }
   $('[id$="hdnSelectedClientClientID"]').val(pClientID);
   $('[id$="hdnReferedPhyID"]').val(pReferingPhysicianID);
   $('[id$="hdfReferalHospitalID"]').val(pLabRefOrgID);
}
...
function CalculationForStatFee(StatFlag) {
   
    if ($('#billPart_divItemTable tr').length > 1) {
        var lengths = $('#billPart_divItemTable tr').length;
        lengths = lengths - 2;
        var FeeType = $('#billPart_divItemTable tr')[lengths].cells[1].innerHTML;
        if (FeeType == 'GEN') {
            var Amt = $('#billPart_divItemTable tr')[lengths].cells[8].innerHTML;
            //var GrossAmt = document.getElementById('billPart_hdnGrossValue').value;
            if (Amt > 0) {
                //var GrossAmt1 = GrossAmt - Amt;
                var TestAmt1 = 0;
                $('[id$="billPart_divItemTable"] tbody tr').each(function(i, n) {
                    var currentRow = $(n);
                    var $row = $(this).closest("tr");
                    if (currentRow.find("td").length > 1) {
                        var RowIndex = $row.index();
                        var FeeID1 = currentRow.find("td")[0].innerHTML;
                        var FeeType1 = currentRow.find("td")[1].innerHTML;
                        var TestAmt = $('#billPart_divItemTable tr')[RowIndex].cells[8].innerHTML;
                        if (FeeType1 != 'GEN' && TestAmt > 0 && FeeID1 != document.getElementById('billPart_hdnAutoCalcGenBillItemID').value && FeeID1 != document.getElementById('billPart_hdnStatAutoFeeID').value) {
                            TestAmt1 = Number(TestAmt1) + Number(TestAmt);
                            //document.getElementById('billPart_hdnGrossValue').value = TestAmt1;
                        }
                    }
                });
                var GrossAmt1 = TestAmt1;
                var StatFeeAmt = GrossAmt1 * Amt / 100;
                StatFeeAmt = (StatFeeAmt).toFixed(2);
                $('#billPart_divItemTable tr')[lengths].cells[8].innerHTML = StatFeeAmt;
                document.getElementById('billPart_hdnStstFee').value = Number(StatFeeAmt).toFixed(2);
                 
//                
//                document.getElementById('billPart_txtGross').value = (Number(StatFeeAmt) + Number(GrossAmt1)).toFixed(2);
//                ToTargetFormat($("#billPart_txtGross"));
//                document.getElementById('billPart_hdnGrossValue').value = document.getElementById('billPart_txtGross').value;
//                ToTargetFormat($("#billPart_hdnGrossValue"));
//                SetNetValue("ADD");
                
                if (document.getElementById('billPart_hdnIsStatValues').value == 'Y' && StatFlag == 'Y' || document.getElementById('hdnNeedAutoStatFee').value == 'Y') {
                    
                    UpdateStatFee(StatFeeAmt);
                }
                CalculatetxtGrossAmt();
            }
        }
    }
}
function UpdateStatFee(StatFeeAmt) {
    var IndexSTATAmount;
    var FeeViewStateValue = document.getElementById('billPart_hdfBillType1').value;
    IndexSTATAmount = FeeViewStateValue.indexOf("STATAmount");
    if (IndexSTATAmount < 0) {
    var test = FeeViewStateValue.substring(0, FeeViewStateValue.length - 1)
    FeeViewStateValue = test + '~STATAmount^' + StatFeeAmt + '|';
    document.getElementById('billPart_hdfBillType1').value = FeeViewStateValue;
    }
}
...
function getOrderedItems() {
    ...
    $('[id$="hdnordereditems"]').val("");
    var rowCount = $('#billPart_divItemTable tr').length;
    if (rowCount > 2) {
        var lstProductItems = [];
        $('[id$="billPart_divItemTable"] tbody tr').each(function(i, n) {
            var currentRow = $(n);
            if (currentRow.find("td").length > 1) {
                var FeeID = currentRow.find("td")[0].innerHTML;
                var FeeType = currentRow.find("td")[1].innerHTML;
                ...
                $('[id$="hdnordereditems"]').val(JSON.stringify(lstProductItems));
            }
        });
    }
}
function minimumPrefixLength() {
    ...
    $find('AutoCompleteExtenderRefPhy').set_minimumPrefixLength(0);
...
function AutoCalcGenralBillItems() {
    $('[id$="billPart_divItemTable"] tbody tr').each(function(i, n) {
        var currentRow = $(n);
        var $row = $(this).closest("tr");
        if (currentRow.find("td").length > 1) {
            ...
            var FeeID = currentRow.find("td")[0].innerHTML;
            var FeeType = currentRow.find("td")[1].innerHTML;
            if (FeeID == document.getElementById('billPart_hdnAutoCalcGenBillItemID').value) {
                var Amt = document.getElementById('billPart_hdnActualGenBillItemAmt').value;
                var TestAmt1 = 0;
                $('[id$="billPart_divItemTable"] tbody tr').each(function(i, n) {
                    var currentRow = $(n);
                    var $row = $(this).closest("tr");
                ...
                var GenBillItems = GrossAmt1 * Amt / 100;
                GenBillItems = (GenBillItems).toFixed(2);
                $('#billPart_divItemTable tr')[RowIndex].cells[8].innerHTML = GenBillItems;
                document.getElementById('billPart_hdnAutoCalcGenBillItemAmt').value = Number(GenBillItems).toFixed(2);
                CalculatetxtGrossAmt();
            }
        }
    });
...
function CalculatetxtGrossAmt() {
    ...
    $('[id$="billPart_divItemTable"] tbody tr').each(function(i, n) {
        ...
        if (currentRow.find("td").length > 1) {
            var RowIndex = $row.index();
            var FeeID = currentRow.find("td")[0].innerHTML;
            if (FeeID == document.getElementById('billPart_hdnStatAutoFeeID').value) {
                document.getElementById('billPart_hdnStstFee').value = $('#billPart_divItemTable tr')[RowIndex].cells[8].innerHTML;
            }
            if (FeeID == document.getElementById('billPart_hdnAutoCalcGenBillItemID').value) {
                document.getElementById('billPart_hdnAutoCalcGenBillItemAmt').value = $('#billPart_divItemTable tr')[RowIndex].cells[8].innerHTML;
            }
            var TestAmt = $('#billPart_divItemTable tr')[RowIndex].cells[8].innerHTML;
            TestAmt1 = Number(TestAmt1) + Number(TestAmt);
        }
    ...
    if (TestAmt1 > 0) {
        document.getElementById('billPart_txtGross').value = TestAmt1;
        document.getElementById('billPart_hdnGrossValue').value = TestAmt1;
