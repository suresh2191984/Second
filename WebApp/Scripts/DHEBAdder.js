...
function AddDHEB(ctlDesc, ctlComments, ctlDeleted, ctlTable, ctlValues, ctlExisitingValues, sDescription, sComment) {
    var unfretval = DHEBValidate(ctlDesc, ctlComments);
    if (unfretval != false) {
        unfCmdAdd_onclick(unfretval, ctlDesc, ctlComments, ctlDeleted, ctlTable, ctlValues, ctlExisitingValues, sDescription, sComment);
    }
}

function DHEBValidate(ctlDesc, ctlComments) {
   
    var ctlDesc1 = document.getElementById(ctlDesc).value;
    var ctlComments = document.getElementById(ctlComments).value;
    if (ctlDesc1 == '' && ctlDesc1 != null) {
        alert('Provide a value');
        ctlDesc.focus();
        return false;
    }
    var descVal = ctlDesc;
    var commentsVal = ctlComments;
    var unfretval = descVal + "~" + commentsVal;
    return unfretval;
}
...
function unfCreateJavaScriptTables(ctlDesc, ctlComments, ctlDeleted, ctlTable, ctlValues, ctlExisitingValues, sDescription, sComment) {
    ...
    unfstartTag = "<TABLE width='100%' ID='tabDrg1' Cellpadding='1' Cellspacing='1' Border='1' style='BackgroundColor:#ff6600;' ><TBODY><tr><td > Select </td><td >" + Col1 + "</td><td >" + Col2 + "</td><TD></TD></tr>";
