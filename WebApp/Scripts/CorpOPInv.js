...
function IAmSelected(source, eventArgs) {
    ...
    for (i = 0; i < x.length; i++) {
        if (x[i] != "") {
            ...
            if (CheckTaskItems(pid + "~" + y[0] + "~" + y[4] + "~" + y[5] + "~" + y[8] + "~" + y[18] + "~" + y[21])) {
...
function checkIsEmpty() {
    ...
    if (Number(document.getElementById('txtBatchQuantity').value) < Number(document.getElementById('txtQuantity').value)) {
        alert('Issued quantity cannot be greater than available quantity');
        document.getElementById('txtQuantity').focus();
        return false;
    }

    if (Number(document.getElementById('txtQuantity').value) < 1) {
        alert('Issued quantity cannot be zero.');
        document.getElementById('txtQuantity').value = "";
        document.getElementById('txtQuantity').focus();
        return false;
    }
...
function BindProductList() {
    ...
    //----------------------------------Check Product Name
        //var Panames = document.getElementById('hdnProductLists').value;
        var x = document.getElementById('hdnProductLists').value.split("^");
        var pCQTY = document.getElementById('txtQuantity').value;
        ...
        var PFlag = 0;
          var quantity = 0;
        var s = document.getElementById('hdnProductList').value.split("^");
        for (i = 0; i < s.length - 1; i++) {
            if (s[i] != "") {
                var y = s[i].split('~');
                if (y[6] == pProductId.trim()) {
                    quantity = quantity + y[3];
                }
            }
        }
        ...
        for (i = 0; i < x.length - 1; i++) {
            if (x[i] != "") {
                ...
                var QID = y[2].trim();
                if (PID == pProductId.trim()) {
                  if ((Number(quantity) +Number(pCQTY)) > QID) {
                        alert("Issued quantity cannot be greater than  ordered qunatity");
                        document.getElementById('txtProduct').value = "";
                        document.getElementById('divProductDetails').style.display = 'none';
                        document.getElementById('txtProduct').focus();
                        return false;
                    }
                    if (Number(QID) >= Number(pCQTY)) {
                        flage1 = 1;
                    }
                    else {
                        PFlag1 = -1;
        ...
        if (flage1 == 1) {
            ...
            alert('Issued quantity cannot be greater than remaining quantiy  :' + ChekQty);
            document.getElementById('txtQuantity').value = "";
            document.getElementById('txtQuantity').focus();
            return false;
        }
        ...
        document.getElementById('chkIsReimburse').style.display = "none";
        if (pHasExpiryDate == "Y") {
...
//    cell6.innerHTML = "Amount";
    cell5.innerHTML = "IsReimbursable";
...
//            cell6.innerHTML = parseFloat(y[3] * y[7]).toFixed(2);
            ...
            cell5.innerHTML = y[11];
            cell6.innerHTML = "<input name='" + y[0] + "~" + y[1] + "~" + y[2] + "~" + y[3] + "~" + y[4] + "~" + y[5] + "~" + y[6] + "~" + y[7] + "~" + y[8] + "~" + y[9] + "~" + y[10] + "~" + y[11] + "~" + y[12] + "~" + y[13] + "' onclick='btnEdit_OnClick(name);' value = 'Edit' type='button' style='background-color:Transparent;color:Red;border-style:none;text-decoration:underline;cursor:pointer;'  /> &nbsp;&nbsp;" +
        ...
        //document.getElementById('chkTaskReasgin').style.display = 'none';
        for (var j = 1; j < document.getElementById('ConsumableItemsTab').rows.length; j++) {
            var Tblrelation = document.getElementById('ConsumableItemsTab');
            var ProductName = Tblrelation.rows[j].cells[0].innerHTML;
            var IssuedQty = Tblrelation.rows[j].cells[2].innerHTML;
            var RQty = y[3];
            if (ProductName == y[1]) {
                Tblrelation.rows[j].style.backgroundColor = "Orange";
                //Tblrelation.rows[j].cells[2].innerHTML = Number(IssuedQty) + Number(RQty);
            }
        }
    }
...
function Deleterows() {
    ...
    var x1 = document.getElementById('hdnProductLists').value.split("^");
    var pCQTY = document.getElementById('txtQuantity').value;
    var pProductId = document.getElementById('hdnProductId').value;
    var flage1 = 0;
    var PFlag1 = 0;
...
function btnDelete(sEditedData) {
    ...
    for (var j = 1; j < document.getElementById('ConsumableItemsTab').rows.length; j++) {
        ...
        var IssuedQty = Tblrelation.rows[j].cells[2].innerHTML;
        if (Productname == y[1]) {
            Tblrelation.rows[j].style.backgroundColor = "white";
            //Tblrelation.rows[j].cells[2].innerHTML = Number(IssuedQty);
        }
    }
...
function checkDetails() {
    if (document.getElementById('chkTaskReasgin').checked == true) {
        return true;
    }
    else {
        var count = document.getElementById('tbllist').rows.length;
        if (document.getElementById('hdnProductList').value == '') {
            ...
            if (tcheck) {
                if (tbacNo.trim().toUpperCase() == y[1].trim().toUpperCase()) {
                    ...
                    if (Number(y[2]) < Number(tIssQty)) {
                       alert('Issued quantity cannot be greater than Available quantiy  :' + ChekQty);
