...
public partial class PurchaseOrder_PurchaseOrderQty : Attune_BasePage
{
    public PurchaseOrder_PurchaseOrderQty()
        : base("PurchaseOrder_PurchaseOrderQty_aspx")
    {
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (NeedOrgLocationBasedGST == "Y")
            {
                ...
                CheckState.Value = lstSOrganizationAdd[0].StateID.ToString();
            }
            else {
                CheckState.Value = StateID.ToString();
            }
            ...
            if (Request.QueryString["ID"] != null && Request.QueryString["Edit"] == "Y")
            {
                ...
                btnDraft.Attributes.Add("class", "hide");

            }
            
            LoadSuppliers();
            ddlSupplierList.SelectedValue = hdnSName.Value;
            lblsupplierName.Text = ddlSupplierList.SelectedItem.Text;
            SetSuppliersDEtails();
            //LoadSellingUnits();
            ...
            string NeedReqSuppProductCostprice = GetConfigValue("NeedSupplierBaseProductcostprice", OrgID);
            hdnNeedReqSuppBaseCostprice.Value = !string.IsNullOrEmpty(NeedReqSuppProductCostprice) ? NeedReqSuppProductCostprice : "N";
        }
        ...
        try
        {
            ...
            List<InventoryItemsBasket> iChildCat = (from i in lstInventoryItemsBasket
                                                    ...
                                                    select new InventoryItemsBasket
                                                    {
                                                        ...
                                                        OrderedUnitValues=g.Key.OrderedUnitValues
                                                    }).Distinct().ToList();
            ...
            ///hdnPurchaseOrderItems.Value = new JavaScriptSerializer().Serialize(lstInventoryItemsBasket2); //tempProductList
            ...
            txtComments.Text = lstPurchaseOrders[0].Comments;
    ...
    public class tempclass
    {
        ...
        public string OrderedUnitValues { get; set; }
    ...
    public void LoadProductList(List<InventoryItemsBasket> lstIIB)
    {
        ...
        List<tempclass> lsttempclass = (from c in lstIIB
                                        select new tempclass
                                        {
                                            ...
                                            Rate =c.UnitSellingPrice,// (c.Quantity * c.Amount) + ((c.Amount * c.Discount) / 100) + ((c.Discount * c.PurchaseTax) / 100),
                                            ProductReceivedDetailsID = c.ProductReceivedDetailsID,
                                            OrderedUnitValues = c.OrderedUnitValues
                                        }).ToList();
    ...
    protected void btnGeneratePO_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            objPO.Comments = txtComments.Text;// hdnComments.Value;
    ...
    private void RemoveDrafts()
    {
        try
        {
            int locationid = InventoryLocationID;
            int intpageID =Convert.ToInt32(hndPageId.Value); 
            InventoryCommon_BL objDraft = new InventoryCommon_BL();
            string DraftValue = ddlSupplierList.SelectedValue;
            objDraft.DeleteDraft(OrgID, locationid, intpageID, LID, "PurchaseOrder", DraftValue);
        }
        catch (Exception ex)
        {
