...
public partial class InventoryReports_StockUsageReport : Attune_BasePage
{
    public InventoryReports_StockUsageReport()
        : base("InventoryReports_StockUsageReport_aspx")
   {
   }
    ...
    private void Loadstockissudreports()
    {
        try
        {
            ...
            new InventoryReports_BL(base.ContextInfo).GetStockUsageReport(sfromdate, sTodate, preProduct, OrgID, ILocationID, FromLocationID, ToLocationID, out lstInventoryItemsBasket);
    ...
    public void ExportToExcel()
    {
        try
        {
            ...
            prefix = Resources.InventoryReports_ClientDisplay.InventoryReports_StockUsageReport_aspx_01;
            ...
            oHtmlTextWriter.WriteLine("<span style='font-size:14.0pt; color:#538ED5;font-weight:700;'> " + Resources.InventoryReports_ClientDisplay.InventoryReports_StockUsageReport_aspx_02 + "</span>");
            grdResult.RenderControl(oHtmlTextWriter);
            oHtmlTextWriter.WriteLine("<span> " +
               " <table  width='100%' style='text-align:right' ><tr><td>" + Resources.InventoryReports_ClientDisplay.InventoryReports_StockUsageReport_aspx_03 + "</td><td>" + lblTotalCP.Text + "</td></tr>" +
                " <tr><td>" + Resources.InventoryReports_ClientDisplay.InventoryReports_StockUsageReport_aspx_04 + "</td><td>" + lblTotalSP.Text + " </td></tr></table>" +
                "</span>");
    ...
    protected void grdResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        try
        {
            ...
            {
                decimal Tcp = Convert.ToDecimal(e.Row.Cells[6].Text) * Convert.ToDecimal(e.Row.Cells[7].Text);
                e.Row.Cells[8].Text = string.Format("{0:0.00}", Tcp);
                TotalCp = TotalCp + Convert.ToDecimal(e.Row.Cells[8].Text);
                TotalSp = TotalSp + Convert.ToDecimal(e.Row.Cells[10].Text);
                lblTotalCP.Text = TotalCp.ToString();
                lblTotalSP.Text = TotalSp.ToString();
                if (e.Row.Cells[12].Text == "01/01/0001")
                {
                    e.Row.Cells[12].Text = "";
                }
