...
public partial class Reports_DeptRevenueDetails : BasePage
{
    long returnCode = -1;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["vdt"] != null)
        {
            ...
            returnCode = new Report_BL(base.ContextInfo).GetDptWiseRevenueReport(visitdate, visitdate, OrgID, 0, Convert.ToInt32(visitType), out lstDWCR, out pTotalItemAmt, out pTotalDiscount, out pTotalReceivedAmt, out pTotalDue, out pTax, out pServiceCharge, "0");
        }
        else if ((Request.QueryString["vfdt"] != null) && (Request.QueryString["vtdt"] != null))
        {
            ...
            returnCode = new Report_BL(base.ContextInfo).GetDptWiseRevenueReport(visitFdate, visitTdate, OrgID, 0, Convert.ToInt32(visitType), out lstDWCR, out pTotalItemAmt, out pTotalDiscount, out pTotalReceivedAmt, out pTotalDue, out pTax, out pServiceCharge, "0");
        }
        if (lstDWCR.Count > 0)
        {
            if (deptName == "CON")
