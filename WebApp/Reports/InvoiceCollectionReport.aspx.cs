...
public partial class Reports_InvoiceCollectionReport : BasePage
{
    public Reports_InvoiceCollectionReport()
        : base("Reports_InvoiceCollectionReport_aspx")
    {
    }
    long returnCode = -1;
    int nOrgID;
    ...
    DateTime nTdate;
    long nClientID;
    List<InvoiceCollectionReport> lstDWCR = new List<InvoiceCollectionReport>();
    List<InvoiceCollectionReport> lstDWCR1 = new List<InvoiceCollectionReport>();
    List<InvoiceCollectionReport> lstDWCR2 = new List<InvoiceCollectionReport>();
    List<InvoiceCollectionReport> lstDWCR3 = new List<InvoiceCollectionReport>();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            hdnClientID.Value = Convert.ToString(ClientID);
            // hdnAdressID.Value = "";
            hdnAdressID.Value = Convert.ToString(OrgAddressID);
            hdnFDate.Value = Convert.ToString(fDate);
            hdnTdate.Value = Convert.ToString(tDate);
            //  hdnOrgID.Value="";
            hdnOrgID.Value = Convert.ToString(pOrgID);
            ...
            int retreiveDataBaseOnVtype = 0;// visitType;
            string strObj = string.Empty;
            List<InvoiceCollectionReport> lstBillwiseDWCR = new List<InvoiceCollectionReport>();
            ...
            //    strObj = "1";
            //}
            returnCode = new Report_BL(base.ContextInfo).GetInvoiceCollectionReportLIMS(
                fDate, tDate, pOrgID, 0, pReportType, ClientID, OrgAddressID,
                out lstDWCR, out lstDWCR1, out lstDWCR2, out lstDWCR3);
            ...
            List<InvoiceCollectionReport> lstDayWiseRept = new List<InvoiceCollectionReport>();
            foreach (var obj in dwcr)
            {
                InvoiceCollectionReport pdc = new InvoiceCollectionReport();
                pdc.OrgID = obj.OrgID;
            ...
            if (lstDWCR.Count > 0)
            {
                gvOrgwiseGrandTotal.DataSource = lstDWCR;
                gvOrgwiseGrandTotal.DataBind();
                decimal total = lstDWCR.Sum(item => item.ReceivedAmount);
                gvOrgwiseGrandTotal.FooterRow.Cells[1].Text = "Total";
                gvOrgwiseGrandTotal.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
                gvOrgwiseGrandTotal.FooterRow.Cells[2].Text = total.ToString("N2");
                lblTotalCollectedAmt.Text = Convert.ToString(total);
                gvCCamount.DataSource = lstDWCR1;
                gvCCamount.DataBind();
                decimal total1 = lstDWCR1.Sum(item => item.ReceivedAmount);
                gvCCamount.FooterRow.Cells[1].Text = "Total";
                gvCCamount.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
                gvCCamount.FooterRow.Cells[2].Text = total1.ToString("N2");

                gvPCAmount.DataSource = lstDWCR2;
                gvPCAmount.DataBind();
                decimal total2 = lstDWCR2.Sum(item => item.ReceivedAmount);
                gvPCAmount.FooterRow.Cells[1].Text = "Total";
                gvPCAmount.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
                gvPCAmount.FooterRow.Cells[2].Text = total2.ToString("N2");

                gvDTSDWR.DataSource = lstDWCR3;
                gvDTSDWR.DataBind();
                decimal total3 = lstDWCR3.Sum(item => item.ReceivedAmount);
                gvDTSDWR.FooterRow.Cells[1].Text = "Total";
                gvDTSDWR.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
                gvDTSDWR.FooterRow.Cells[2].Text = total3.ToString("N2");

                lblDiscountAmount.Text = Convert.ToString(lstDWCR3[0].ReceivedAmount);
                lblTDS.Text = Convert.ToString(lstDWCR3[1].ReceivedAmount);
                lblWriteOFF.Text = Convert.ToString(lstDWCR3[2].ReceivedAmount);
                lblCredit.Text = "0.00";
                lblTotalPendingAMount.Text = "0.00";
                Panel1.Style.Add("display", "block");
                pnlDetailedReport.Style.Add("display", "none");
            }
            else
            {
    ...
    protected void grdCollection_ItemCommand(object source, GridViewCommandEventArgs e)
    {

        try
        {
            nFdate = Convert.ToDateTime(hdnFDate.Value);
            nTdate = Convert.ToDateTime(hdnTdate.Value);
            nOrgID = Convert.ToInt32(hdnOrgID.Value);
            nOrgAddressID = Convert.ToInt32(hdnAdressID.Value);
            nClientID = Convert.ToInt64(hdnClientID.Value);
            base.ContextInfo.LocationID = 0;
            if (e.CommandName == "Client")
                nClientID = Convert.ToInt64(e.CommandArgument);
            if (e.CommandName == "User")
                lID = Convert.ToInt64(e.CommandArgument);
            if (e.CommandName == "PaymentType")
                base.ContextInfo.UserID = Convert.ToInt64(e.CommandArgument);
            if (e.CommandName == "Type")
                base.ContextInfo.LocationID = Convert.ToInt32(e.CommandArgument);
            long returnCode = new Report_BL(base.ContextInfo).GetInvoiceCollectionReportLIMS(
                        nFdate, nTdate, nOrgID, lID, e.CommandName, nClientID, nOrgAddressID,
                        out lstDWCR, out lstDWCR1, out lstDWCR2, out lstDWCR3);
            grdDetailedReport.DataSource = lstDWCR;
            grdDetailedReport.DataBind();

           // decimal total = lstDWCR.Sum(item => item.ReceivedAmount);
            grdDetailedReport.FooterRow.Cells[6].Text = "Total";
            grdDetailedReport.FooterRow.Cells[6].HorizontalAlign = HorizontalAlign.Right;
           // grdDetailedReport.FooterRow.Cells[7].Text = total.ToString("N2");
            decimal InvoiceAmount = lstDWCR.Sum(item => item.InvoiceAmount);
           // grdDetailedReport.FooterRow.Cells[8].HorizontalAlign = HorizontalAlign.Right;
            grdDetailedReport.FooterRow.Cells[7].Text = InvoiceAmount.ToString("N2");

            decimal Discount = lstDWCR.Sum(item => item.Discount);
           // grdDetailedReport.FooterRow.Cells[9].HorizontalAlign = HorizontalAlign.Right;
            grdDetailedReport.FooterRow.Cells[8].Text = Discount.ToString("N2");
            decimal TDS = lstDWCR.Sum(item => item.TDS);
            //grdDetailedReport.FooterRow.Cells[9].HorizontalAlign = HorizontalAlign.Right;
            grdDetailedReport.FooterRow.Cells[9].Text = TDS.ToString("N2");

            decimal WriteOffAmount = lstDWCR.Sum(item => item.WriteOffAmount);
            grdDetailedReport.FooterRow.Cells[10].Text = WriteOffAmount.ToString("N2");
            decimal ReceivedAmount = lstDWCR.Sum(item => item.ReceivedAmount);
            grdDetailedReport.FooterRow.Cells[11].Text = ReceivedAmount.ToString("N2");
            decimal PendingAmount = lstDWCR.Sum(item => item.PendingAmount);
            grdDetailedReport.FooterRow.Cells[14].Text = PendingAmount.ToString("N2");
            ///  ModalPanel.Visible = true; 
            //  mpe.Show();
          //  lblTotalCollectedAmt1.Text = Convert.ToString(total);
            //lblDiscountAmount1.Text = Convert.ToString(lstDWCR[0].ReceivedAmount);
            //lblTDS1.Text = Convert.ToString(lstDWCR[1].ReceivedAmount);
            //lblWriteOFF1.Text = Convert.ToString(lstDWCR[2].ReceivedAmount);
            //lblCredit1.Text = "0.00";
            //lblTotalPendingAMount1.Text = "0.00";
            Panel1.Style.Add("display", "none");
            pnlDetailedReport.Style.Add("display", "block");
    ...
    {
        gvOrgwiseGrandTotal.PageIndex = e.NewPageIndex;
        //this.BindGrid();
    }
    protected void gvIPCreditMainGrandTotal_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //List<InvoiceCollectionReport> lstday = new List<InvoiceCollectionReport>();
                //List<InvoiceCollectionReport> lstday1 = new List<InvoiceCollectionReport>();
                //InvoiceCollectionReport RMaster = (InvoiceCollectionReport)e.Row.DataItem;
                //InvoiceCollectionReport TotalVal = new InvoiceCollectionReport();
                ...
                ////                 select child;
                ////}
                //foreach (InvoiceCollectionReport obj in childItems.ToList())
                //{
    ...
    protected void gvOrgwiseGrandTotal_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[6].Text += "<br/><b style='color:green;'>" + strcan + " " + Convert.ToDecimal(e.Row.Cells[8].Text).ToString() + " </b>" + "<br/><b style='color:green;'>" + strreceived + (Convert.ToDecimal(e.Row.Cells[6].Text) - Convert.ToDecimal(e.Row.Cells[8].Text)).ToString() + "</b>";
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvOrgwisePatientSummary_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                List<InvoiceCollectionReport> lstDateWiseRept = new List<InvoiceCollectionReport>();
                Label lblOrgID = (Label)e.Row.FindControl("lblOrgID");
    ...
    public DataTable loaddata(List<InvoiceCollectionReport> lstDWCR)
    {
        ...
        foreach (InvoiceCollectionReport item in lstDWCR)
        {
    ...
    public void ExportToExcel()
    {
        try
        {
            ...
            // Panel1.
            //   gvCCamount.RenderControl(oHtmlTextWriter);
    ...
    protected void gvPCAmount_SelectedIndexChanged(object sender, EventArgs e)
    {
