...
public partial class Reports_DueReport : BasePage
{
    long returnCode = -1;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            if (hdnReportType.Value == "DL")
            {
                ...
                if (ddlTrustedOrg.Items.Count > 0)
                    OrgID = Convert.ToInt32(ddlTrustedOrg.SelectedValue);
                returnCode = new Report_BL(base.ContextInfo).GetDueandDiscount(fDate, tDate, OrgID, visitType, "DUE", PName, PNo, out lstDWCR, out pTotalDueAmt);
                ...
                if (lstDWCR.Count > 0)
                {
                    ...
                    if (lstDWCR.Count > 0)
                    {
                        ...
                        if (Rbltypeofpatient.SelectedItem.Value == "Creditandcash")
                        {
                            totDueAmt = lstDWCR.FindAll(p => p.IsCreditBill == "Y" || p.IsCreditBill == "N").Sum(p => p.TotalAmount);
                        ...
                        else if (Rbltypeofpatient.SelectedItem.Value == "Credit")
                        {
                            ...
                            totDueAmt = lstDWCR.FindAll(p => p.IsCreditBill == "Y").Sum(p => p.TotalAmount);
                        ...
                        {
                            totDueAmt = lstDWCR.FindAll(p => p.IsCreditBill == "N").Sum(p => p.TotalAmount);
                ...
                tdtypeofpatient.Attributes.Add("style", "Display:block");
    ...
    private void BindDuePaidReport()
    {
        try
        {
            ...
            tdtypeofpatient.Attributes.Add("style", "Display:none");
