...
public partial class Reports_PendingsamplesReport : BasePage
{
    long returnCode = -1;
    List<PendingsamplesReport> lstFiftySamples = new List<PendingsamplesReport>();
    List<PendingsamplesReport> lstSamples = new List<PendingsamplesReport>();
    ...
    public void LoadGridFifty()
    {
        try
        {
            ...
            // returnCode = new ReportExcel_BL(base.ContextInfo).GetPendingsamplesReport(OrgID, strBillFromDate, strBillToDate, out lstSamples);
            grdResultFifty.Width = 775;
            grdResultFifty.Visible = true;
            grdResultFifty.DataSource = lstFiftySamples;
            grdResultFifty.DataBind();

            if (lstSamples.Count > 2)
            {
    ...
    public void LoadGrid()
    {
        try
        {
            ...
            strBillFromDate = Convert.ToDateTime(txtFrom.Text);
            strBillToDate = Convert.ToDateTime(txtTo.Text);
            returnCode = new ReportBusinessLogic.ReportExcel_BL(base.ContextInfo).GetPendingsamplesReport(OrgID, strBillFromDate, strBillToDate, out lstFiftySamples, out lstSamples);
            grdResult.Width = 775;
            grdResult.Visible = true;
            grdResult.DataSource = lstSamples;
            grdResult.DataBind();
    ...
    protected void grdResult_RowDataBound(Object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label lnkFeeDescription = (Label)e.Row.FindControl("lnkSamplereceivedAt");
                if (lnkFeeDescription.Text == "Avg")
                {
        ...
        {
            CLogger.LogError("Error while Loading PendingSamples Details", Ex);
        }
    ...
    public void ExportToExcel()
    {
        try
        {
            string str = "Pending for the reporting sample (>4 HRS)";
    ...
    protected void btnPrintAll_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            sb.Append(",top=0,width=825,height=600,status=0,scrollbars=1');");
    ...
    protected void btnSendMailReport_Click(object sender, EventArgs e)
    {
        ...
        //        {
        //            ReturnCode = Communication.SendMail(EmailTo, "", "", "Pending for the reporting sample (>4 HRS)", sw.ToString() + "<div style='font-family:Verdana;font-size:12;'><p></p><div><br><br>Sincerely,<br><strong><br>" + OrgName + "<br/>" + "</strong></div></div>", lstMailAttachment, oMailConfig);
        //        }
        //    }
    ...
    protected void grdResultFifty_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                //  e.Row.BackColor = System.Drawing.Color.PapayaWhip;
                e.Row.Font.Size = 8;
                Label lnkFeeDescription = (Label)e.Row.FindControl("lnkFiftySamplereceivedAt");
                if (lnkFeeDescription.Text == "Sum Avg")
                {
                    e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                    e.Row.Font.Bold = true;
                    //  e.Row.BackColor = System.Drawing.Color.Wheat;
                ...
                if (lnkFeeDescription.Text == "Total Avg")
                {
                    e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Right;
                    e.Row.Font.Bold = true;
                    //     e.Row.BackColor = System.Drawing.Color.Wheat;
